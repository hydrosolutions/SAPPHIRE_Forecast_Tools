#!/usr/bin/env bash
# ============================================================================
# export_hydrograph_period_history.sh
#
# LAPTOP-SIDE export wrapper for P2b (hydrograph PENTAD/DECADE history).
#
...
# from the ``hydrographs`` table filtered on:
...
print_usage() {
    cat <<'USAGE'
Usage: bash bin/export_hydrograph_period_history.sh [OPTIONS]

LAPTOP-SIDE export of hydrograph PENTAD or DECADE history from the local
sapphire-preprocessing-db into a CSV + manifest pair, for transfer to a
deployment server via scp. Pair with `bin/initialize_hydrograph_period_history.sh`
...
#
# Testing-only bypass: set ``_P2B_EXPORT_SKIP_LOCATION_GUARD=1`` in the
...
location_guard() {
    ...
    if [[ -n "$detected" ]]; then
        ...
        echo "    bash bin/initialize_hydrograph_period_history.sh <env_file> ..." >&2
        exit 1
    fi
}
...
main() {
    ...
    local csv_path="${OUT_DIR}/hydrograph_period_${HORIZON}_${ts}.csv"
    ...
    echo -e "${BOLD}========================================"
    echo -e " HYDROGRAPH ${horizon_upper} EXPORT (P2b)"
    echo -e "========================================${NC}"
...
SQL
    ...
    # The SELECT lists ALL writable HydrographBase columns; the CSV column
    # names match the API payload keys so the server-side wrapper does NOT
    # need year-column discovery (year_map). The lowercase horizon_type form
    # is emitted as 'pentad' / 'decade' to match the payload convention.
    local copy_sql
    copy_sql=$(cat <<SQL
COPY (
...
# Generated by bin/export_hydrograph_period_history.sh on $(date -u +%Y-%m-%dT%H:%M:%SZ)
export_type=hydrograph_period
horizon=${HORIZON}
row_count=${row_count}
...
MANIFEST
    ...
    echo "  bash bin/initialize_hydrograph_period_history.sh \\"
