...
#
# Hydrograph DAY CSV-to-API migration wrapper (P3 of the update-time migration
# toolkit). Reads <data_ref>/intermediate_data/hydrograph_day.csv (a wide CSV
# with stat/quantile columns plus year-named columns like '2025'/'2026') and
# POSTs records to the preprocessing API.
#
# Uses the P0 helper (bin/utils/update_migration_helpers.sh) for image
# resolution, temp workspace, redacted logging, and the uniform image log line.
...
#
# CONTRAST vs the legacy data_migrator.py:
#   The old in-service migrator hardcoded year columns '2024'/'2025'
#   (sapphire/services/preprocessing/app/data_migrator.py:264-265). Year
#   columns rotate every January (next-year may also exist), so the hardcode
#   silently mismaps after each new year. This wrapper performs dynamic
#   year-column discovery in migration_py.hydrograph_day._discover_year_columns
#   and prints the inferred mapping in the dry-run inventory.
#
# Universal safe-write rule (architecture Q2 layer 2):
#   Hydrograph rows have many nullable stat/quantile/year fields. The wrapper
#   sends ONLY non-NULL fields from the source CSV. The service-side
#   _has_changes + setattr path overwrites existing non-NULL with incoming
#   NULL, so the wrapper never injects null. Choice trade-off vs explicit
#   read-before-merge documented in the P3 gi_draft (Option A vs Option B).
#
...
# binding flag name reused by P1b / P1c and now P3.
#
# Usage:
#   bash bin/initialize_hydrograph_day_history.sh <env_file_path> [OPTIONS]
#
# Arguments:
...
#
# Examples:
#   bash bin/initialize_hydrograph_day_history.sh /data/taj_data/config/.env_tjhm --dry-run
#   bash bin/initialize_hydrograph_day_history.sh /data/taj_data/config/.env_tjhm
#   bash bin/initialize_hydrograph_day_history.sh /data/taj_data/config/.env_tjhm \
...
print_usage() {
    cat <<'USAGE'
Usage: bash bin/initialize_hydrograph_day_history.sh <env_file_path> [OPTIONS]

Migrate hydrograph DAY rows from intermediate_data/hydrograph_day.csv into the
preprocessing API. Honors MODE=full-import vs pre-cutoff based on the target
table state. Performs DYNAMIC year-column discovery (the source CSV's year
columns rotate every January) and prints the inferred
HYDROGRAPH_YEAR_MAPPING={previous: <year>, current: <year>} mapping in the
dry-run inventory.
...
query_target_state() {
    docker exec sapphire-preprocessing-db psql \
        ...
        -c "SELECT COUNT(*), COALESCE(MIN(date)::text, '') FROM hydrographs WHERE horizon_type='DAY';"
}
...
main() {
    ...
    print_banner
    echo "| Running Hydrograph DAY History Initialization (CSV -> API push)"
    ...
    # Derived paths.
    local CSV_HOST="${ieasyhydroforecast_data_ref_dir}/intermediate_data/hydrograph_day.csv"
    local LOG_DIR="${ieasyhydroforecast_data_root_dir}/logs/hydrograph_day_history_init"
    mkdir -p "${LOG_DIR}"

    local TIMESTAMP
    TIMESTAMP="$(date -u +%Y%m%dT%H%M%SZ)"
    log_file="${LOG_DIR}/hydrograph_day_history_init_${TIMESTAMP}.log"
    export log_file

    echo "| Log file: ${log_file}"
    echo ""

    umh_log_redacted "Starting hydrograph DAY history initialization"
    ...
    # -------------------------------------------------------------------------
    local TMPDIR_HYDRO
    TMPDIR_HYDRO="$(umh_acquire_temp_workspace hydrograph_day)"
    ...
    umh_log_redacted "========================================"
    umh_log_redacted "Running hydrograph DAY push via docker"
    umh_log_redacted "  image:        ${IMAGE}"
    umh_log_redacted "  csv:          ${CSV_HOST} -> /hydrograph_day.csv (ro)"
    ...
    docker run --rm --network host \
        -v "${CSV_HOST}:/hydrograph_day.csv:ro" \
        -v "${HELPER_DIR}/migration_py:/opt/migration_py:ro" \
        -e "PYTHONPATH=/opt" \
        "$IMAGE" \
        python3 -m migration_py.hydrograph_day \
            --csv-path /hydrograph_day.csv \
            --api-url "${API_URL}" \
    ...
    echo ""
    echo -e "${BOLD}========================================"
    echo -e " HYDROGRAPH DAY HISTORY INIT COMPLETE"
    echo -e "========================================${NC}"
    echo ""
    ...
    echo "    \"SELECT horizon_type, COUNT(*) AS rows, MIN(date), MAX(date) FROM hydrographs WHERE horizon_type='DAY' GROUP BY horizon_type;\""
    echo ""
    echo "Log file: ${log_file}"
