...
# pre-cutoff (cutoff = MIN(date)). The query is scoped to the mode's
# horizon_value so different modes get independent cutoffs.
#
...
based on the target table state (queried per mode's horizon_value).
...
# query is unscoped by horizon_value here; per-mode cutoffs would require a
# per-mode query loop. The pre-cutoff cutoff is conservative — any mode that
# has overlapping dates with the target table will be filtered identically.
# ---------------------------------------------------------------------------
query_target_state() {
