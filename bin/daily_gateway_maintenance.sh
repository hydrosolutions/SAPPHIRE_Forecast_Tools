#!/bin/bash
# Daily Preprocessing Gateway Maintenance Script
#
# This script runs the preprocessing gateway module in maintenance mode to fill
# gaps in the SAPPHIRE API. It should be run weekly or when API data issues are
# suspected.
#
# In maintenance mode, the three gateway scripts write a lookback window to the
...
#
# Usage:
#   bash bin/daily_gateway_maintenance.sh <env_file_path>
#
# Example:
#   bash bin/daily_gateway_maintenance.sh /path/to/config/.env
...
# Print the banner
print_banner
echo "| Running Preprocessing Gateway Maintenance Mode"
...
LOG_DIR="${ieasyhydroforecast_data_root_dir}/logs/gateway_maintenance"
mkdir -p ${LOG_DIR}
...
log_message "Starting Preprocessing Gateway maintenance run"
...
CONTAINER_NAME="prepgateway-maintenance"
SERVICE_LOG="${LOG_DIR}/${CONTAINER_NAME}_${TIMESTAMP}.log"

log_message "Starting preprocessing gateway in maintenance mode..."
...
# Run the preprocessing gateway container in maintenance mode.
...
if [ "$CONTAINER_EXIT_CODE" -eq 0 ]; then
    log_message "Preprocessing Gateway maintenance completed successfully"
else
    log_message "WARNING: Preprocessing Gateway maintenance completed with exit code: $CONTAINER_EXIT_CODE"
...
log_message "Preprocessing Gateway maintenance run completed"
echo "|"
