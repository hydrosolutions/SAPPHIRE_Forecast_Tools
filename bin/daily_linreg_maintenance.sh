#!/bin/bash
# Daily Linear Regression Maintenance Script
#
# This script runs the linear regression module in hindcast mode to catch up
# on any missed forecasts. It should be run nightly after the regular forecast
# workflow completes.
#
# Usage:
...
# 2. Run linear regression hindcast for both PENTAD and DECAD modes
# 3. Auto-detect the start date from existing output files
...
# Print the banner
print_banner
echo "| Running Linear Regression Maintenance (Hindcast Mode)"
...
LOG_DIR="${ieasyhydroforecast_data_root_dir}/logs/linreg_maintenance"
mkdir -p ${LOG_DIR}
...
log_message "Starting Linear Regression maintenance run"
...
IMAGE_ID="mabesa/sapphire-linreg:${ieasyhydroforecast_backend_docker_image_tag:-latest}"
...
# Define prediction modes to run
...
for MODE in "${PREDICTION_MODES[@]}"; do
    CONTAINER_NAME="linreg-maintenance-${MODE}"
    SERVICE_LOG="${LOG_DIR}/${CONTAINER_NAME}_${TIMESTAMP}.log"

    log_message "Starting hindcast for $MODE mode..."
...
log_message "Linear Regression maintenance run completed"
echo "|"
echo "| All hindcast runs complete. Check logs at: $LOG_DIR"
echo "|"
