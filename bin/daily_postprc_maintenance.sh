#!/bin/bash
# Daily Postprocessing Maintenance Script
#
# This script runs the postprocessing forecasts module in maintenance mode
# to fill gaps in ensemble forecasts. It detects (date, code) pairs where
# individual model forecasts exist but ensemble (EM) is missing, then
# creates the missing ensembles from pre-calculated skill metrics.
#
# Usage:
#   bash bin/daily_postprc_maintenance.sh <env_file_path>
#
# Example:
#   bash bin/daily_postprc_maintenance.sh /path/to/config/.env
#
# The script will:
# 1. Read configuration from the .env file
# 2. Run postprocessing in maintenance mode (7-day lookback by default)
# 3. Fill missing ensemble forecasts
...
# Print the banner
print_banner
echo "| Running Postprocessing Forecasts Maintenance Mode"
...
LOG_DIR="${ieasyhydroforecast_data_root_dir}/logs/postprc_maintenance"
mkdir -p ${LOG_DIR}
...
log_message "Starting Postprocessing Forecasts maintenance run"
...
CONTAINER_NAME="postprc-maintenance"
SERVICE_LOG="${LOG_DIR}/${CONTAINER_NAME}_${TIMESTAMP}.log"

log_message "Starting postprocessing forecasts in maintenance mode..."
...
# Run the postprocessing maintenance container
docker run \
    --name $CONTAINER_NAME \
    --network host \
    ...
    ${IMAGE_ID} \
    uv run postprocessing_maintenance.py \
    2>&1 | tee "$SERVICE_LOG"
...
if [ "$CONTAINER_EXIT_CODE" -eq 0 ]; then
    log_message "Postprocessing maintenance completed successfully"
else
    log_message "WARNING: Postprocessing maintenance completed with exit code: $CONTAINER_EXIT_CODE"
...
log_message "Postprocessing Forecasts maintenance run completed"
echo "|"
