...
class TestTraceFeb20Pipeline:
    ...
    @pytest.fixture
    def daily_data(self):
        return _make_daily_discharge(self.CODES, "2022-01-01", "2026-02-23", base=50.0)
    ...
    def test_stage1_pentad_identity(self):
        ...
        print(
            f"\n[Stage 1] Feb 20 pentad_in_year={pentad_in_year}, pentad_in_month={pentad_in_month}"
        )
        # get_pentad_in_year returns a string
        assert str(pentad_in_year) == "10", f"Expected pentad '10', got {pentad_in_year!r}"
    ...
    def test_stage2_generate_dates(self, daily_data):
        ...
        feb20 = data_pentad[
            (data_pentad["date"] == pd.Timestamp("2026-02-20")) & (data_pentad["code"] == "S1")
        ]
        ...
        assert bool(row["issue_date"]) is True, "Feb 20 should be issue_date=True"
        assert pd.notna(row["discharge_sum"]), "discharge_sum is NaN! This means 3-day sum failed."
        assert pd.notna(row["discharge_avg"]), (
            "discharge_avg is NaN! This means pentadal avg failed."
        )
    ...
    def test_stage3_filter_data(self, daily_data):
        ...
        feb20 = discharge_pentad[(discharge_pentad["date"] == pd.Timestamp("2026-02-20"))]
        print("\n[Stage 3] Feb 20 rows after filter (both stations):")
        print(
            feb20[
                ["date", "code", "discharge_sum", "discharge_avg", "issue_date", "pentad_in_year"]
            ].to_string()
        )
        ...
        for _, row in feb20.iterrows():
            assert pd.notna(row["discharge_sum"]), f"discharge_sum NaN for {row['code']}"
            assert pd.notna(row["discharge_avg"]), f"discharge_avg NaN for {row['code']}"
    ...
    def test_stage4_perform_linear_regression(self, daily_data):
        ...
        print(f"[Stage 4] Result years: {sorted(result['date'].dt.year.unique())}")
        ...
        if not result_2026.empty:
            ...
            print(
                f"[Stage 4] ALL pentad_in_year values in data: "
                f"{sorted(discharge_pentad['pentad_in_year'].unique())}"
            )
            # Show the last few rows per station
            for code in self.CODES:
                code_data = discharge_pentad[discharge_pentad["code"] == code].tail(5)
                print(f"\n[Stage 4] Last 5 rows for {code}:")
                print(
                    code_data[
                        ["date", "pentad_in_year", "discharge_sum", "discharge_avg", "issue_date"]
                    ].to_string()
                )
        ...
        issue_2026 = result_2026[result_2026["issue_date"]]
    ...
    def test_stage5_write_function(self, daily_data, tmp_path):
        ...
        issue_rows = result[result["issue_date"]]
        ...
        with patch.object(fl, "_write_lr_forecast_to_api"):
            fl.write_linreg_pentad_forecast_data(result, forecast_date=self.FORECAST_DATE)

        assert output_path.exists(), (
            "CSV file was NOT written! This means the write function "
            "returned early. Check stale-data validation."
        )
        ...
        assert all(written["date"] == "2026-02-20"), f"Dates in CSV: {written['date'].tolist()}"
    ...
    def test_stage6_discharge_avg_detail(self):
        ...
        result = fl.calculate_pentadaldischargeavg(data, "date", "discharge")

        print("\n[Stage 6] After calculate_pentadaldischargeavg:")
        print(result[["date", "discharge", "issue_date", "discharge_avg"]].to_string())

        # Check Feb 20 specifically
        feb20 = result[result["date"] == pd.Timestamp("2026-02-20")]
        print(f"\n[Stage 6] Feb 20 discharge_avg: {feb20['discharge_avg'].values}")

        assert len(feb20) == 1
        avg_val = feb20["discharge_avg"].iloc[0]
        print(f"[Stage 6] discharge_avg value: {avg_val}, is_nan: {pd.isna(avg_val)}")

        # Also check Feb 15 (issue_date=True, day 15)
        feb15 = result[result["date"] == pd.Timestamp("2026-02-15")]
        if not feb15.empty:
            print(f"[Stage 6] Feb 15 discharge_avg: {feb15['discharge_avg'].values}")
    ...
    def test_stage7_discharge_sum_detail(self):
        ...
        print(result[["date", "discharge", "issue_date", "discharge_sum"]].to_string())

        feb20 = result[result["date"] == pd.Timestamp("2026-02-20")]
        print(f"\n[Stage 7] Feb 20 discharge_sum: {feb20['discharge_sum'].values}")
        assert len(feb20) == 1
        sum_val = feb20["discharge_sum"].iloc[0]
        assert pd.notna(sum_val), "discharge_sum is NaN for Feb 20!"
    ...
    def test_stage8_data_ends_at_feb20(self, tmp_path):
        ...
        daily = _make_daily_discharge(self.CODES, "2022-01-01", "2026-02-20", base=50.0)
        ...
        result_2026 = result[(result["date"].dt.year == 2026) & result["issue_date"]]
        print("\n[Stage 8] Regression output for 2026 issue dates:")
        if not result_2026.empty:
            print(
                result_2026[
                    ["date", "code", "discharge_sum", "discharge_avg", "forecasted_discharge"]
                ].to_string()
            )
        else:
            ...
            print(
                all_2026[
                    ["date", "code", "discharge_sum", "discharge_avg", "issue_date"]
                ].to_string()
            )
        ...
        if not result.empty:
            ...
            if output_path.exists():
                ...
                pytest.fail("Write function skipped output when data ends at Feb 20")
