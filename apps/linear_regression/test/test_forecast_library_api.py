    ...
    def test_api_enabled_uses_api(self, mock_api_read):
        ...
        try:
            mock_api_read.return_value = pd.DataFrame(
                {"code": ["12345"], "date": pd.to_datetime(["2024-01-01"]), "discharge": [10.5]}
            )
...
class TestWriteConsistency:
    ...
    @patch("forecast_library._write_lr_forecast_to_api")
    def test_pentad_csv_and_api_data_match(
        self, mock_api_write, sample_forecast_data, temp_env_for_consistency
    ):
        ...
        try:
            ...
            for code in api_codes:
                ...
                csv_row = csv_data[csv_data["code"].astype(str) == code].iloc[
                    -1
                ]  # Last row for this code
    ...
    @patch("forecast_library.SapphirePostprocessingClient")
    def test_api_record_structure_matches_csv_columns(
        self, mock_client_class, sample_forecast_data, temp_env_for_consistency
    ):
        ...
        try:
            ...
            for record in api_records:
                ...
                if record["forecasted_discharge"] is not None:
                    assert (
                        abs(record["forecasted_discharge"] - csv_row["forecasted_discharge"])
                        < 0.001
                    )
...
class TestReadConsistency:
    ...
    def test_csv_and_api_return_same_data_values(self, temp_env_for_read, sample_discharge_csv):
        ...
        for i, (csv_val, api_val) in enumerate(zip(csv_discharge, api_discharge, strict=True)):
            assert abs(csv_val - api_val) < 0.001, (
                f"Discharge mismatch at index {i}: CSV={csv_val}, API={api_val}"
            )
    ...
    def test_csv_and_api_return_same_date_types(self, temp_env_for_read, sample_discharge_csv):
        ...
        assert pd.api.types.is_datetime64_any_dtype(csv_result["date"]), (
            "CSV date should be datetime"
        )
        assert pd.api.types.is_datetime64_any_dtype(api_result["date"]), (
            "API date should be datetime"
        )
    ...
    def test_unified_function_returns_consistent_format(
        self, temp_env_for_read, sample_discharge_csv
    ):
...
class TestWriteHydrographToApi:
    ...
    @patch("forecast_library.SapphirePreprocessingClient")
    def test_invalid_horizon_type_raises_error(
        self, mock_client_class, sample_pentad_hydrograph_data
    ):
        """Invalid horizon_type should raise ValueError."""
        if not fl.SAPPHIRE_API_AVAILABLE:
            pytest.skip("sapphire-api-client not installed")
...
class TestWriteRunoffToApi:
            ...
            data = pd.DataFrame(
                {
                    "code": ["12345", "67890", "12345", "67890"],
                    "date": pd.to_datetime(
                        ["2024-01-01", "2024-01-01", "2024-01-05", "2024-01-05"]
                    ),
                    "pentad": [1, 1, 1, 1],
                    "pentad_in_year": [1, 1, 1, 1],
                    "discharge_avg": [100.0, 101.0, 102.0, 103.0],
    ...
    def test_default_sync_mode_is_operational(self, mock_client_class):
        ...
        try:
            ...
            fl._write_runoff_to_api(data, "pentad")
