...
def _setup_common_mocks(mock_sl, mock_fl, mock_tl, mock_sdk_hf,
    ...
    mock_sl.ForecastFlags.from_forecast_date_get_flags.return_value = (
        MagicMock(pentad=True, decad=True)
    )

    fake_sites = [_make_fake_site('15013')]
    mock_sl.get_pentadal_forecast_sites_from_HF_SDK.return_value = (
        fake_sites, ['15013'], None
    )
    mock_sl.get_decadal_forecast_sites_from_HF_SDK.return_value = (
        fake_sites, ['15013'], None
    )
    ...
    mock_fl.parse_dates_robust.return_value = pd.to_datetime(
        pd.Series(['2024-01-05'])
    )
...
class TestMainForecastMode:
    ...
    def test_forecast_mode_calls_store_last_run_date(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
        ...
        mock_sl.define_run_dates.return_value = (
            forecast_date, forecast_date, forecast_date + dt.timedelta(days=1)
        )
    ...
    @patch.object(lr_module, 'fl')
    @patch.object(lr_module, 'sl')
    def test_forecast_mode_pentad_only(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
    ...
    @patch.object(lr_module, 'fl')
    @patch.object(lr_module, 'sl')
    def test_forecast_mode_decad_only(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
...
class TestMainHindcastMode:
    ...
    @patch.object(lr_module, 'fl')
    @patch.object(lr_module, 'sl')
    def test_hindcast_does_not_store_last_run_date(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
    ...
    @patch.object(lr_module, 'fl')
    @patch.object(lr_module, 'sl')
    def test_hindcast_skips_non_forecast_days(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
    ...
    @patch.object(lr_module, 'fl')
    @patch.object(lr_module, 'sl')
    def test_hindcast_up_to_date_exits_cleanly(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
...
class TestMainEarlyExits:
    ...
    @patch.object(lr_module, 'fl')
    @patch.object(lr_module, 'sl')
    def test_exits_when_no_forecast_date(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
    ...
    @patch.object(lr_module, 'fl')
    @patch.object(lr_module, 'sl')
    def test_exits_when_no_pentad_data(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
        """Empty pentad DataFrame with PENTAD mode → exits."""
        _setup_common_mocks(mock_sl, mock_fl, mock_tl, mock_sdk_hf)
        mock_sl.define_run_dates.return_value = (
            dt.date(2024, 1, 5), dt.date(2024, 1, 5), dt.date(2024, 1, 6)
        )
        # Return empty pentad data
        mock_fl.get_pentadal_and_decadal_data.return_value = (
            pd.DataFrame(), _make_data_df()
        )
    ...
    @patch.object(lr_module, 'fl')
    @patch.object(lr_module, 'sl')
    def test_exits_when_hindcast_autodetect_fails(
        self, mock_sl, mock_fl, mock_tl, mock_sdk_hf
    ):
        ...
        with patch.object(
            lr_module, 'get_hindcast_start_date_from_output',
            return_value=None,
        ):
            env = {**_BASE_ENV, 'SAPPHIRE_PREDICTION_MODE': 'PENTAD'}
            with patch('sys.argv', [
