...
def _make_daily_discharge(station_codes, start_date, end_date, base_value=50.0):
...
class TestFeb20DataFlow:
    ...
    def test_stage1_daily_data_has_feb20(self, daily_data):
        ...
        assert len(feb20) == 2, f"Expected 2 rows for Feb 20 (one per station), got {len(feb20)}"
    ...
    def test_stage2_add_pentad_issue_date(self, daily_data):
        ...
        for code in ["S1", "S2"]:
            ...
            assert bool(feb20["issue_date"].iloc[0]) is True, (
            ...
            assert bool(feb19["issue_date"].iloc[0]) is False, (
    ...
    def test_stage4_discharge_avg(self, daily_data):
        ...
        for code in ["S1", "S2"]:
            station_data = daily_data[daily_data["code"] == code].copy()
            station_data = fl.add_pentad_issue_date(station_data, "date")
            station_data = fl.calculate_3daydischargesum(station_data, "date", "discharge")
            result = fl.calculate_pentadaldischargeavg(station_data, "date", "discharge")

            feb20 = result[result["date"] == pd.Timestamp("2026-02-20")]
            assert len(feb20) == 1, f"Feb 20 row missing after discharge_avg for {code}"
            # Check if discharge_avg is NaN or has a value
            avg_val = feb20["discharge_avg"].iloc[0]
            # Log for diagnostics
            print(f"\n[{code}] Feb 20 discharge_avg = {avg_val} (NaN={pd.isna(avg_val)})")
    ...
    def test_stage5_generate_issue_and_forecast_dates(self, daily_data):
        ...
        # Check Feb 20 exists
        feb20 = data_pentad[data_pentad["date"] == pd.Timestamp("2026-02-20")]
        assert len(feb20) == 2, (
    ...
    def test_stage6_filter_for_feb20(self, daily_data):
        ...
        pentad10 = discharge_pentad[discharge_pentad["pentad_in_year"] == 10.0]

        # Check if Feb 20, 2026 is in the result
        feb20 = pentad10[pentad10["date"] == pd.Timestamp("2026-02-20")]
        date_max = pentad10["date"].max()
        print(f"\n[Stage 6] date_max in pentad10 = {date_max}")
        print(f"[Stage 6] Feb 20 rows = {len(feb20)}")
        print(f"[Stage 6] Last 5 dates in pentad10: {sorted(pentad10['date'].unique())[-5:]}")

        assert len(feb20) == 2, (
            f"Expected 2 rows for Feb 20 in pentad10 filter, got {len(feb20)}. date_max={date_max}"
        )
    ...
    def test_stage7_issue_date_filter_retains_feb20(self, daily_data):
        ...
        # Filter for issue_date rows (mimics write function)
        issue_rows = pentad10[pentad10["issue_date"]]

        # Check Feb 20, 2026
        feb20 = issue_rows[issue_rows["date"] == pd.Timestamp("2026-02-20")]
        print(
        ...
        assert len(feb20) == 2, f"Expected 2 rows for Feb 20 with issue_date=True, got {len(feb20)}"
    ...
    def test_stage8_dropna_effect(self, daily_data):
        ...
        for code in ["S1", "S2"]:
            ...
            feb20_before = station_data[station_data["date"] == pd.Timestamp("2026-02-20")]
            ...
            if len(feb20_before) > 0:
                ...
                nan_cols = [col for col in row.index if pd.isna(row[col])]
...
class TestWriteSkipsStaleData:
    ...
    def test_pentad_skips_when_no_current_year_data(self, mock_api):
        ...
        fl.write_linreg_pentad_forecast_data(data, forecast_date=dt.date(2026, 2, 20))

        assert not os.path.exists(self.pentad_path), "CSV should NOT be written when data is stale"
    ...
    def test_pentad_writes_when_current_year_data(self, mock_api):
        ...
        assert os.path.exists(self.pentad_path), "CSV should be written when data is current"
    ...
    def test_decad_skips_when_no_current_year_data(self, mock_api):
        ...
        fl.write_linreg_decad_forecast_data(data, forecast_date=dt.date(2026, 2, 20))
    ...
    def test_decad_writes_when_current_year_data(self, mock_api):
        ...
        assert os.path.exists(self.decad_path), "Decad CSV should be written when data is current"
...
class TestPerformLinearRegressionForecastDate:
    ...
    def test_backward_compat_no_forecast_date(self):
        ...
        result = fl.perform_linear_regression(
            data, "station", "pentad", "discharge_sum", "discharge_avg", 2
        )
