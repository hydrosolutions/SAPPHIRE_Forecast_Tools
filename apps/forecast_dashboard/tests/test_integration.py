...
def _fetch_summary_table_data_from_db(
    ...
    if horizon in ("month", "season"):
        ...
        if not rows:
            return {}
        # Take the latest forecast date — that's what the Summary table shows.
        max_date = max(r["date"] for r in rows if r.get("date"))
        latest = [r for r in rows if r.get("date") == max_date]
        ...
        for r in latest:
            ...
            # /long-forecast/ uses field `q` for forecasted discharge.
            forecasted = r.get("q")
            if forecasted is None:
                forecasted = r.get("forecasted_discharge")
            merged[model_short]["forecasted_discharge"] = forecasted
        return merged
