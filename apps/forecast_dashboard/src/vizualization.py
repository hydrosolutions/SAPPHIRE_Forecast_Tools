...
def make_frame_attribution_hook(
    text,
    corner="top_left",
    x_offset=8,
    y_offset=8,
    ...
    def hook(plot, element):
        ...
        try:
            cb_code = (
                ...
                if (left_anchor and top_anchor)
                else "lbl.x = xr.end; lbl.y = yr.end;"
                if (not left_anchor and top_anchor)
                else "lbl.x = xr.start; lbl.y = yr.start;"
                if (left_anchor and not top_anchor)
                else "lbl.x = xr.end; lbl.y = yr.start;"  # bottom_right
            ) + " lbl.change.emit();"

            for attr in ("start", "end"):
                fig.x_range.js_on_change(
                    attr, CustomJS(args=dict(lbl=lab, xr=fig.x_range, yr=fig.y_range), code=cb_code)
                )
...
def plot_runoff_line(data, date_col, line_data_col, label_text, color):
    ...
    # Create the curve
    line = hv.Curve(data, kdims=[date_col], vdims=[line_data_col], label=label_text).opts(
        color=color, line_width=2, tools=["hover"], show_legend=True
    )
...
def plot_runoff_forecasts(
    ...
    if len(models) > len(runoff_forecast_colors):
        ...
        runoff_forecast_color = runoff_forecast_colors + [
            ...
            for i in range(len(models) - len(runoff_forecast_colors))
        ]
        ...
        runoff_forecast_color = [
            runoff_forecast_colors[1],
            runoff_forecast_colors[3],
            runoff_forecast_colors[-2],
        ]
        line_types = ["solid", "dashed", "dotdash"]
    elif len(models) == 4:
        runoff_forecast_color = [
            runoff_forecast_colors[0],
            runoff_forecast_colors[2],
            runoff_forecast_colors[4],
            runoff_forecast_colors[-1],
        ]
...
def plot_runoff_range_area(data, date_col, min_col, max_col, range_legend_entry, range_color):
    """
    ...
    """
    range_area = hv.Area(
        data, kdims=[date_col], vdims=[min_col, max_col], label=range_legend_entry
    ).opts(
        color=range_color,
        alpha=0.5,
        muted_alpha=0.1,
...
def plot_runoff_forecast_range_area(
    ...
    for i, model in enumerate(models):
        ...
        range_area = hv.Area(
            model_data, kdims=[date_col], vdims=[min_col, max_col], label=legend_entry
        ).opts(
            color=runoff_forecast_color[i],
            alpha=0.2,
            muted_alpha=0.05,
...
def create_stepped_area_plot(x, y1, y2, range_color, legend_entry):
    ...
    df = pd.DataFrame({"x": xs, "y1": y1s, "y2": y2s})

    # Create area plot
    area = hv.Area(df, kdims=["x"], vdims=["y2", "y1"], label=legend_entry)
...
def plot_current_runoff_forecast_range(
    ...
    for i, model in enumerate(models):
        ...
        upper_bound = fl.round_discharge(model_data[max_col].iloc[-1])
        range_legend_entry = (
            model + " " + _("range") + ": " + lower_bound + "-" + upper_bound + " " + unit_string
        )
        # print(f"Debug: model_data\n{model_data}")
        # print("MODEL: ", model)
        # print("COLOR: ", runoff_forecast_color[i])
        ...
        # Create a point
        point = hv.Scatter(
            model_data, kdims=[date_col], vdims=[mean_col], label=point_legend_entry
        ).opts(color=runoff_forecast_color[i], size=7, tools=["hover"], show_legend=True)

        range_segment = hv.ErrorBars(model_data, label=range_legend_entry).opts(
            color=runoff_forecast_color[i],
            alpha=0.2,
            line_width=4,
            show_legend=True,
...
def plot_current_runoff_forecast_range_date_format(
    ...
    jitter_timedelta = pd.to_timedelta(
        np.linspace(-jitter_width, jitter_width, len(data)), unit="h"
    )
...
def plot_current_runoff_forecast_range_date_format_v2(
    ...
    for i, model in enumerate(models):
        ...
        if model == "LR":
            ...
            point = hv.Curve(
                model_data_temp, kdims=[date_col], vdims=[mean_col], label=point_legend_entry
            ).opts(
                color=runoff_forecast_color[i],
                line_width=2,
                line_dash=line_types[i],
            ...
            range_segment = plot_runoff_range_area(
                ...
                range_legend_entry,
                runoff_forecast_color[i],
            ).opts(alpha=0.2, muted_alpha=0.05)
...
def plot_runoff_range_bound(data, date_col, range_bound_col, range_color, hover_tool=True):
    ...
    if hover_tool:
        boundary_line = hv.Curve(data, kdims=[date_col], vdims=[range_bound_col]).opts(
            color=range_color, line_width=0, line_alpha=0, show_legend=False, tools=["hover"]
        )
    else:
        boundary_line = hv.Curve(data, kdims=[date_col], vdims=[range_bound_col]).opts(
            color=range_color, line_width=0, line_alpha=0, show_legend=False, tools=[]
        )
...
def create_cached_vlines(_, horizon, for_dates=True, y_text=1):
    ...
    if for_dates:
        ...
        vlines = hv.Path(path_data).opts(
            color="gray", line_width=1, line_dash="dotted", line_alpha=0.5, show_legend=False
        )
...
def plot_daily_hydrograph_data(
    ...
    hvspan_predictor = hv.VSpan(
        ...
        color=runoff_current_year_color, alpha=0.2, line_width=0, muted_alpha=0.05, show_legend=True
    )
    ...
    def make_hover_bounds(data, date_col, low_col, high_col, color):
        def add_hover_bounds(plot, element):
            ...
            # Hover tools
            hover_high = HoverTool(
                renderers=[renderer_high],
                tooltips=[("Date", "$x{%F}"), (high_col, "$y{0.1f}")],
                formatters={"$x": "datetime"},
            )
            hover_low = HoverTool(
                renderers=[renderer_low],
                tooltips=[("Date", "$x{%F}"), (low_col, "$y{0.1f}")],
                formatters={"$x": "datetime"},
            )
    ...
    full_range_area = plot_runoff_range_area(
        ...
        tools=[],
        hooks=[make_hover_bounds(data, date_col, min_col, max_col, runoff_full_range_color)],
    )
    ...
    area_05_95 = plot_runoff_range_area(
        ...
        q05_col,
        q95_col,
        _("90-percentile range legend entry"),
        runoff_90percentile_range_color,
    ).opts(
        tools=[],
        hooks=[
            make_hover_bounds(data, date_col, q05_col, q95_col, runoff_90percentile_range_color)
        ],
    )
    ...
    area_25_75 = plot_runoff_range_area(
        ...
        q25_col,
        q75_col,
        _("50-percentile range legend entry"),
        runoff_50percentile_range_color,
    ).opts(
        tools=[],
        hooks=[
            make_hover_bounds(data, date_col, q25_col, q75_col, runoff_50percentile_range_color)
        ],
    )

    mean = plot_runoff_line(data, date_col, mean_col, _("Mean legend entry"), runoff_mean_color)
    last_year = plot_runoff_line(
        data, date_col, last_year_col, _("Last year legend entry"), runoff_last_year_color
    )
...
def plot_daily_rainfall_data(_, wm, daily_rainfall, station, date_picker, linreg_predictor):
    ...
    if station_data.empty:
        return hv.Curve([]).opts(
            title=_("No data available for this station"), hooks=[remove_bokeh_logo]
        )
    ...
    predictor_rainfall = current_year[
        (current_year["date"] >= predictor_start_date)
    ...
    current_year_text = (
        f"{_(f'Current year, {current_period}: ')}{predictor_rainfall['P'].sum().round()} mm"
    )
    forecast_text = (
        f"{_(f'Precipitation forecast, {forecast_period}: ')} {forecasts['P'].sum().round()} mm"
    )
    ...
    hv_norm_rainfall.opts(interpolation="steps-mid", color=runoff_mean_color, show_legend=True)
    # A bar plot for the current_year rainfall
    hv_current_year = hv.Curve(current_year, kdims="date", vdims="P", label=current_year_text)
    hv_current_year.opts(
        interpolation="steps-mid", color=runoff_current_year_color, show_legend=True
    )
    ...
    if not forecasts.empty:
        hv_forecast = hv.Curve(forecasts, kdims="date", vdims="P", label=forecast_text)
        hv_forecast.opts(
            interpolation="steps-mid", color=runoff_forecast_color_list[3], show_legend=True
        )
        figure = (
            hvspan_predictor
            * hvspan_forecast
...
def plot_daily_temperature_data(_, wm, daily_rainfall, station, date_picker, linreg_predictor):
    ...
    current_year_text = (
        f"{_('Current year')}, {current_period}: {predictor_rainfall['T'].mean()} °C"
    )
    ...
    hv_norm_rainfall.opts(interpolation="linear", color=runoff_mean_color, show_legend=True)
    # A bar plot for the current_year rainfall
    hv_current_year = hv.Curve(current_year, kdims="date", vdims="T", label=current_year_text)
    hv_current_year.opts(interpolation="linear", color=runoff_current_year_color, show_legend=True)
    # Build an attribution hook that pins the label to the plot frame
    attribution_hook = make_frame_attribution_hook(
        text="Data: ECMWF IFS HRES via Open Data",
        corner="top_left",
    ...
    # if forecasts is not empty
    if not forecasts.empty:
        hv_forecast = hv.Curve(forecasts, kdims="date", vdims="T", label=forecast_text)
        hv_forecast.opts(
            interpolation="linear", color=runoff_forecast_color_list[3], show_legend=True
        )
        figure = (
            hvspan_predictor
            * hvspan_forecast
    ...
    figure.opts(
        ...
        xformatter=DatetimeTickFormatter(days="%b %d", months="%b %d"),
        ylim=(
            min([station_data["T"].min(), station_data["T_norm"].min()]) * 0.9,
...
def plot_daily_snow_data(_, wm, snow_data, variable, station, date_picker, linreg_predictor):
    ...
    if daily_snow is None or daily_snow.empty:
        return hv.Curve([]).opts(
            title=_(f"No {variable} data available"), hooks=[remove_bokeh_logo]
        )

    # Variable-specific settings
    variable_config = {
        "SWE": {
            "label": _("Snow Water Equivalent"),
            "unit": "mm",
            "ylabel": _("SWE (mm)"),
            "decimals": 1,
        },
        "HS": {
            "label": _("Snow Height"),
            "unit": "m",
            "ylabel": _("Snow Height (m)"),
            "decimals": 2,
        },
        "RoF": {
            "label": _("Snowmelt Runoff & P Runoff"),
            "unit": "mm",
            "ylabel": _("Snowmelt & P Runoff (mm)"),
            "decimals": 1,
        },
    }
    config = variable_config.get(
        variable, {"label": variable, "unit": "", "ylabel": variable, "decimals": 1}
    )
    ...
    current_year_text = (
        f"{_('Current year')}, {current_period}: {mean_value:.{decimals}f} {config['unit']}"
    ...
    forecast_text = (
        f"{_('Forecast')}, {forecast_period}: {forecast_mean:.{decimals}f} {config['unit']}"
    ...
    hv_norm.opts(interpolation="linear", color=runoff_mean_color, show_legend=True)

    # Current year curve
    hv_current_year = hv.Curve(current_year, kdims="date", vdims=variable, label=current_year_text)
    hv_current_year.opts(interpolation="linear", color=runoff_current_year_color, show_legend=True)

    # Forecast curve (if forecasts exist)
    if not forecasts.empty:
        hv_forecast = hv.Curve(forecasts, kdims="date", vdims=variable, label=forecast_text)
        hv_forecast.opts(
            interpolation="linear", color=runoff_forecast_color_list[3], show_legend=True
        )
        figure = (
            hvspan_predictor * hvspan_forecast * vlines * hv_norm * hv_current_year * hv_forecast
        )
    else:
...
def plot_pentad_forecast_hydrograph_data_v2(
    ...
    forecasts = forecasts_all[
        (forecasts_all["station_labels"] == station)
    ...
    if range_type == _("delta"):
        if "Q25" in forecasts.columns and "Q75" in forecasts.columns:
            ...
            forecasts["fc_lower"] = forecasts["Q25"].where(
                ~forecasts["Q25"].isna(), forecasts["forecasted_discharge"] - forecasts["delta"]
            )
            forecasts["fc_upper"] = forecasts["Q75"].where(
                ~forecasts["Q75"].isna(), forecasts["forecasted_discharge"] + forecasts["delta"]
            )
        else:
        ...
        forecasts.loc[:, "fc_lower"] = np.maximum(
            forecasts.loc[:, "forecasted_discharge"] - forecasts.loc[:, "delta"],
            (1 - range_slider / 100.0) * forecasts.loc[:, "forecasted_discharge"],
        )
        forecasts.loc[:, "fc_upper"] = np.minimum(
            forecasts.loc[:, "forecasted_discharge"] + forecasts.loc[:, "delta"],
            (1 + range_slider / 100.0) * forecasts.loc[:, "forecasted_discharge"],
        )
    ...
    forecasts_current = forecasts[
        forecasts["date"] == pd.to_datetime((title_date) + dt.timedelta(days=1))
    ]
    forecasts_past = forecasts[
        forecasts["date"] <= pd.to_datetime((title_date) + dt.timedelta(days=1))
    ]
    ...
    if rram_forecast is not None:
        ...
        latest_rram_forecast[horizon_in_month] = (
            forecasts_current[horizon_in_month].values[0] if not forecasts_current.empty else None
        )
        latest_rram_forecast[horizon_in_year] = (
            forecasts_current[horizon_in_year].values[0] if not forecasts_current.empty else None
        )
    ...
    if ml_forecast is not None:
        ...
        latest_ml_forecast[horizon_in_month] = (
            forecasts_current[horizon_in_month].values[0] if not forecasts_current.empty else None
        )
        latest_ml_forecast[horizon_in_year] = (
            forecasts_current[horizon_in_year].values[0] if not forecasts_current.empty else None
        )
    ...
    lower_bound = plot_runoff_range_bound(data, date_col, min_col, runoff_full_range_color)
    upper_bound = plot_runoff_range_bound(data, date_col, max_col, runoff_full_range_color)
    ...
    line_05 = plot_runoff_range_bound(data, date_col, q05_col, runoff_90percentile_range_color)
    line_95 = plot_runoff_range_bound(data, date_col, q95_col, runoff_90percentile_range_color)
    ...
    line_25 = plot_runoff_range_bound(data, date_col, q25_col, runoff_50percentile_range_color)
    line_75 = plot_runoff_range_bound(data, date_col, q75_col, runoff_50percentile_range_color)
    ...
    forecast_lower_bound = plot_runoff_range_bound_v2(
        forecasts_past,
        _("date"),
        _("forecast lower bound column name"),
        runoff_forecast_color_list[3],
        hover_tool=True,
    )
    ...
    forecast_line = plot_runoff_forecasts_v2(
        ...
        _("forecast model short column name"),
        runoff_forecast_color_list,
        _("m³/s"),
    )
    ...
    if (
        "TFT" in forecasts_current[_("forecast model short column name")].values
        or "TiDE" in forecasts_current[_("forecast model short column name")].values
        or "TSMixer" in forecasts_current[_("forecast model short column name")].values
        or "NE" in forecasts_current[_("forecast model short column name")].values
        or "ARIMA" in forecasts_current[_("forecast model short column name")].values
        or "EM" in forecasts_current[_("forecast model short column name")].values
    ):
...
def plot_pentad_forecast_hydrograph_data(
    ...
    if horizon == "pentad":
        ...
        # Define strings
        title_text = (
            f"{_('Hydropost')} {station}: {_('Forecast')} {_('for')} "
    ...
    full_range_area = plot_runoff_range_area(
        ...
        _("min column name"),
        _("max column name"),
        _("Full range legend entry"),
        runoff_full_range_color,
    )
    lower_bound = plot_runoff_range_bound(
        data, horizon_column_name, _("min column name"), runoff_full_range_color, hover_tool=False
    )
    upper_bound = plot_runoff_range_bound(
        data, horizon_column_name, _("max column name"), runoff_full_range_color, hover_tool=False
    )
    ...
    norm = plot_runoff_line(
        data, horizon_column_name, _("norm column name"), _("Norm legend entry"), runoff_norm_color
    )
    mean = plot_runoff_line(
        data, horizon_column_name, _("mean column name"), _("Mean legend entry"), runoff_mean_color
    )
    """last_year = plot_runoff_line(
    ...
    if range_visibility == "Yes":
        ...
        pentad_hydrograph = (
            norm * mean * current_year * forecast_line * current_forecast_range_point
        )
...
def create_forecast_summary_table(
    ...
    if hasattr(date_picker, "value"):
        forecast_table = (
            forecast_table[
                ...
                & (forecast_table["model_short"].isin(all_models))
            ]
            .copy()
        ...
        forecast_table = (
            forecast_table[
                (forecast_table["date"] <= (pd.Timestamp(date_picker) + pd.Timedelta(days=1)))
                & (forecast_table["model_short"].isin(all_models))
            ]
            .copy()
            .reset_index(drop=True)
    ...
    if not forecast_table.empty and not forecast_table["date"].empty:
        ...
        if not pd.isna(max_date):
            forecast_table = forecast_table.loc[
                forecast_table["date"] == max(forecast_table["date"])
            ]
        else:
    ...
    final_forecast_table = forecast_table.sort_values(by=[_("Model")]).copy().reset_index(drop=True)
...
pentads = [
    f"{i + 1}{_('st pentad of')} {calendar.month_name[month]}"
    if i == 0
    else f"{i + 1}{_('nd pentad of')} {calendar.month_name[month]}"
    if i == 1
    else f"{i + 1}{_('rd pentad of')} {calendar.month_name[month]}"
    if i == 2
    else f"{i + 1}{_('th pentad of')} {calendar.month_name[month]}"
...
pentad_options = {
    ...
    if i == 2
    else f"{i + 1}{_('th pentad of')} {calendar.month_name[month]}": i + (month - 1) * 6 + 1
    for month in range(1, 13)
    for i in range(6)
}
...
decad_options = {
    f"{i + 1}{_('st decade of')} {calendar.month_name[month]}"
    if i == 0
    else f"{i + 1}{_('nd decade of')} {calendar.month_name[month]}"
    if i == 1
    else f"{i + 1}{_('rd decade of')} {calendar.month_name[month]}"
    if i == 2
    else f"{i + 1}{_('th decade of')} {calendar.month_name[month]}": i + (month - 1) * 3 + 1
    for month in range(1, 13)
    for i in range(3)
}
...
SSH_TUNNEL_SCRIPT_PATH = env.get(
    "SSH_TUNNEL_SCRIPT_PATH", "../../../sensitive_data_forecast_tools/bin/.ssh/open_ssh_tunnel.sh"
)
...
if os.getenv("IN_DOCKER_CONTAINER"):
    ...
    SSH_TUNNEL_SCRIPT_PATH = re.sub(
        r"open_ssh_tunnel.sh", "open_ssh_tunnel_docker.sh", SSH_TUNNEL_SCRIPT_PATH
    )
...
def select_and_plot_data(
    ...
    if os.path.exists(save_file_path):
        ...
        if (
            "slope" in saved_forecast_table.columns
            and "intercept" in saved_forecast_table.columns
            and "rsquared" in saved_forecast_table.columns
        ):
        ...
        forecast_table["visible"] = (~forecast_table["predictor"].isna()) & (
            ~forecast_table["discharge_avg"].isna()
        )
        # Store the initial visible data points
        initial_visible_data = forecast_table[forecast_table["visible"] == True].copy()
        initial_visible_data = initial_visible_data.dropna(subset=["predictor", "discharge_avg"])
    ...
    forecast_data_table = pn.widgets.Tabulator(
        value=forecast_table_display[
            ["index", _("year"), _("predictor"), _("discharge average"), _("visible")]
        ],
        theme="bootstrap",
        editors={_("visible"): CheckboxEditor()},  # Checkbox editor for the 'visible' column
        formatters={_("visible"): BooleanFormatter(), _("year"): NumberFormatter(format="0")},
    ...
    def update_plot(event=None):
        ...
        forecast_table.loc[forecast_data_table.value[_("index")], "visible"] = (
            forecast_data_table.value[_("visible")].values
        )
        ...
        if visible_data.empty:
            ...
            scatter = hv.Scatter(
                visible_data, kdims="predictor", vdims=["discharge_avg", "year"]
            ).opts(
                xlabel=_("predictor"),
                ylabel=_("discharge_avg"),
                color="blue",
                size=5,
                tools=[hover, "tap"],
                xlim=x_range,
            ...
            if len(visible_data) > 1:
                ...
                def create_html_content(equation_initial, r2_initial, equation_new, r2_new):
                    ...
                    html_content += (
                        f"<span style='color: red;'>{DASHED_LINE}</span> {equation_new}<br>{r2_new}"
                    )
                    html_content += "</div>"
                    return html_content
                ...
                line_new = hv.Curve((x, y_new)).opts(
                    color="red", line_width=2, line_dash="dashed", line_alpha=0.7
                )
                ...
                html_content = create_html_content(
                    equation_initial, r2_initial, equation_new, r2_new
                )
                    ...
                    def hook(plot, element):
                        ...
                        def add_equation_labels(line, equation, r2, y):
                            ...
                            r2_label = Label(
                                x=x_pos + (fig.x_range.end - fig.x_range.start) * 0.04,
                                y=y
                                - (fig.y_range.end - fig.y_range.start)
                                * 0.04,  # Slightly below the equation
                                x_units="data",
                                y_units="data",
                                text=r2,
                ...
                plot = scatter.opts(width=1000, height=450, hooks=[remove_bokeh_logo])
    ...
    def run_docker_container(
        client, full_image_name, volumes, environment, container_name, progress_bar
    ):
        """
        ...
        SSH_TUNNEL_SCRIPT_ABSOLUTE = os.path.join(
            get_absolute_path(os.getenv("ieasyhydroforecast_bin_path")), SSH_TUNNEL_SCRIPT_PATH
        )
        ...
        try:
            with establish_ssh_tunnel(SSH_TUNNEL_SCRIPT_ABSOLUTE):
                ...
                try:
                    ...
                    print(
                        f"Removing existing container '{container_name}' (ID: {existing_container.id})..."
                    )
                ...
                if platform.system() == "Darwin":
                    print("In select_and_plot_data: platform.system() == 'Darwin'")
                    environment.extend(
                        [
                    ...
                    print("In select_and_plot_data: platform.system() == 'Linux'")
                    environment.extend(
                        [
                ...
                container = client.containers.run(
                    ...
                    name=container_name,
                    network_mode="host",
                    extra_hosts={"host.docker.internal": "host-gateway", "localhost": "127.0.0.1"},
                )
                ...
                if result["StatusCode"] != 0:
                    print(
                        f"Container '{container_name}' exited with status code {result['StatusCode']}."
                    )
    ...
    def save_to_csv(event):
        ...
        updated_forecast_table = updated_forecast_display.rename(
            columns={
                _("year"): "year",
                _("predictor"): "predictor",
        ...
        pn.state.onload(
            lambda: pn.state.add_periodic_callback(
                lambda: setattr(popup, "visible", False), 2000, count=1
            )
        )
        ...
        try:
            ...
            volumes = {
                ...
                absolute_volume_path_internal_data: {
                    "bind": bind_volume_path_internal_data,
                    "mode": "rw",
                },
            ...
            print(
                f"Time taken to run reset_rundate: {temp_docker_end - start_docker_runs:.2f} seconds"
            )
            ...
            temp_docker_end = time.time()
            print(
                f"Time taken to run postprocessing: {temp_docker_end - temp_docker_start:.2f} seconds"
            )
        ...
        pn.state.onload(
            lambda: pn.state.add_periodic_callback(
                lambda: setattr(progress_bar, "visible", False), 2000, count=1
            )
        )
        pn.state.onload(
            lambda: pn.state.add_periodic_callback(
                lambda: setattr(progress_message, "visible", False), 4000, count=1
            )
        )
...
def create_reload_button(horizon):
    ...
    loading_spinner = pn.indicators.LoadingSpinner(
        value=True, width=50, height=50, color="success", visible=False
    )
    progress_message = pn.pane.Alert("Processing...", alert_type="info", visible=False)
    warning_message = pn.pane.Alert(
        "Please do not reload this page until processing is done!",
        alert_type="danger",
        visible=False,
    )
    ...
    def run_pipeline(event):
        ...
        def run_docker_pipeline():
            try:
                ...
                start_docker_runs = time.time()
                run_docker_container(
                    client, "mabesa/sapphire-preprunoff:latest", volumes, environment, "preprunoff"
                )
                temp_docker_end = time.time()
                print(
                    f"Time taken to run preprunoff: {temp_docker_end - start_docker_runs:.2f} seconds"
                )
                temp_docker_start = time.time()

                # Run the reset_rundate container
                run_docker_container(
                    client, "mabesa/sapphire-rerun:latest", volumes, environment, "reset_rundate"
                )
                temp_docker_end = time.time()
                print(
                    f"Time taken to run reset_rundate: {temp_docker_end - temp_docker_start:.2f} seconds"
                )
                temp_docker_start = time.time()

                # Run the linear_regression container
                run_docker_container(
                    client, "mabesa/sapphire-linreg:latest", volumes, environment, "linreg"
                )
                temp_docker_end = time.time()
                print(
                    f"Time taken to run linreg: {temp_docker_end - temp_docker_start:.2f} seconds"
                )
                temp_docker_start = time.time()
                ...
                temp_docker_end = time.time()
                print(
                    f"Time taken to run ML models: {temp_docker_end - temp_docker_start:.2f} seconds"
                )
                temp_docker_start = time.time()
                ...
                progress_message.object = _(
                    "Processing finished in {overall_time:.2f} seconds"
                ).format(overall_time=overall_time)
                time.sleep(2)
...
def run_docker_container(client, full_image_name, volumes, environment, container_name):
    ...
    if not os.path.isfile(SSH_TUNNEL_SCRIPT_ABSOLUTE):
        ...
        SSH_TUNNEL_SCRIPT_ABSOLUTE = os.path.join(
            os.getenv("ieasyhydroforecast_container_data_ref_dir"), "bin", SSH_TUNNEL_SCRIPT_PATH
        )
        if not os.path.isfile(SSH_TUNNEL_SCRIPT_ABSOLUTE):
            print(f"SSH tunnel script not found at: {SSH_TUNNEL_SCRIPT_ABSOLUTE}")
            return
...
def plot_forecast_skill(
    ...
    forecast_pentad = (
        forecast_pentad.sort_values("date", ascending=False)  # Sort by date, most recent first
        .groupby(["model_short", horizon_in_year])  # Group by model and pentad/decad
        .apply(
            lambda x: x.dropna(subset=["sdivsigma", "accuracy"]).head(1)
    ...
    hv_06a = hv.Area(
        pd.DataFrame({"x": [1, area_x_lim], "y": [0.6, 0.6]}),
        kdims=["x"],
        vdims=["y"],
        label=_("Effectiveness") + " <= 0.6",
    ).opts(alpha=0.05, color="green", line_width=0)
    hv_08a = hv.Area(
        pd.DataFrame({"x": [1, area_x_lim], "y": [0.8, 0.8]}),
        kdims=["x"],
        vdims=["y"],
    ...
    effectiveness_plot = (
        hv_08a * hv_06a * hv_forecast_skill_effectiveness * hv_current_forecast_skill_effectiveness
    )
    effectiveness_plot.opts(
        ...
        xlabel=horizon_x_label,
        ylabel=_("Effectiveness [-]"),
        show_grid=True,
        ylim=(0, 1.4),
        fontsize={"legend": 8, "title": 10},
        fontscale=1.2,
    )
...
def create_skill_table(_, horizon, forecast_stats):
    ...
    filters = {
        ...
        _("Month"): {"type": "list", "values": unique_months, "placeholder": _("Filter by month")},
        _("Model"): {"type": "list", "values": unique_models, "placeholder": _("Filter by model")},
        _("Code"): {"type": "list", "values": unique_codes, "placeholder": _("Filter by code")},
    }
