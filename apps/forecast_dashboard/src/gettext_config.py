...
def load_translation(language, locale_dir):
    ...
    gettext.bindtextdomain('forecast_dashboard', locale_dir)
    gettext.textdomain('forecast_dashboard')
    # Print the path to the .mo file
    print(f"debug load_translation: path to .mo file: {gettext.find('forecast_dashboard', locale_dir, languages=[language])}")
    return gettext.translation('forecast_dashboard', locale_dir, languages=[language]).gettext
...
# xgettext -o ../config/locale/messages.pot forecast_dashboard.py
# 2. Create a new translation file, make sure you have a backup of the old one
# to avoid having to translate everything again.
# msginit -i ../config/locale/messages.pot -o ../config/locale/ru_KG/LC_MESSAGES/forecast_dashboard.po -l ru_KG
# msginit -i ../config/locale/messages.pot -o ../config/locale/en_CH/LC_MESSAGES/forecast_dashboard.po -l en_CH
# 3. Translate the strings in the .po file and make sure that charset is set to
# UTF-8 (charset=UTF-8)
# 4. Compile the .po file to a .mo file
# msgfmt -o ../config/locale/ru_KG/LC_MESSAGES/forecast_dashboard.mo ../config/locale/ru_KG/LC_MESSAGES/forecast_dashboard.po
# msgfmt -o ../config/locale/en_CH/LC_MESSAGES/forecast_dashboard.mo ../config/locale/en_CH/LC_MESSAGES/forecast_dashboard.po
