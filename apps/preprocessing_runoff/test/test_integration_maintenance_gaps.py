...
class TestMaintenanceModeDataGaps:
    ...
    def test_env(self, tmp_path):
        ...
        original_env = {
            "ieasyforecast_intermediate_data_path": os.environ.get(
                "ieasyforecast_intermediate_data_path"
            ),
            "ieasyforecast_daily_discharge_path": os.environ.get(
                "ieasyforecast_daily_discharge_path"
            ),
            "ieasyforecast_daily_discharge_file": os.environ.get(
                "ieasyforecast_daily_discharge_file"
            ),
            "ieasyforecast_hydrograph_day_file": os.environ.get(
    ...
    def _create_history_csv_with_gap(
        self, output_path, site_codes, start_date, end_date, gap_start, gap_end
    ):
        """
    ...
    def test_merge_alone_preserves_gaps_outside_api_range(self, test_env):
        ...
        result = src._merge_with_update(cached_data, api_data, "code", "date", "discharge")
    ...
    def test_hydrograph_reflects_data_gaps(self, test_env):
        ...
        hydrograph = src.from_daily_time_series_to_hydrograph(
            data_df=runoff_df.copy(), date_col="date", discharge_col="discharge", code_col="code"
        )
...
class TestGapDetection:
        ...
        data = pd.DataFrame(
            {
                "code": ["A", "A", "A"],
                "date": pd.to_datetime(
                    [base_date, base_date + timedelta(days=2), base_date + timedelta(days=3)]
                ),
        ...
        data = pd.DataFrame(
            {
                "code": ["A", "A", "A"],
                "date": pd.to_datetime(
                    [base_date, base_date + timedelta(days=1), base_date + timedelta(days=7)]
                ),
        ...
        data = pd.DataFrame(
            {
                ...
                "date": pd.to_datetime(
                    [
                        base_date,
                        base_date + timedelta(days=2),
                        base_date + timedelta(days=3),  # Site A: gap on day 1
                        base_date,
                        base_date + timedelta(days=1),
                        base_date + timedelta(days=3),  # Site B: gap on day 2
                    ]
                ),
    ...
    def test_max_gap_age_respected(self):
        ...
        data = pd.DataFrame(
            {"code": ["A", "A"], "date": [old_date, recent_date], "discharge": [10.0, 20.0]}
        )
        ...
        cached_data = pd.DataFrame(
            {
                "code": ["A", "A", "A"],
                "date": pd.to_datetime(
                    [base_date, base_date + timedelta(days=1), base_date + timedelta(days=6)]
                ),
...
class TestEndToEndMaintenanceModeGapFilling:
    ...
    def test_fill_gaps_from_api_calls_sdk_correctly(self, test_env):
        ...
        gaps = [
            (
                "16059",
                pd.Timestamp(base_date + timedelta(days=5)),
                pd.Timestamp(base_date + timedelta(days=8)),
            ),
            (
                "15189",
                pd.Timestamp(base_date + timedelta(days=5)),
        ...
        with patch.object(
            src,
            "get_daily_average_discharge_from_iEH_HF_for_multiple_sites",
            ...
            assert sites_in_result == {"16059", "15189"}, (
                f"Expected both sites, got {sites_in_result}"
            )
    ...
    def test_maintenance_mode_no_gaps_no_api_calls(self, test_env):
        ...
        detected_gaps = src._detect_gaps_in_data(
            continuous_data, "code", "date", max_gap_age_days=730
        )
        ...
        with patch.object(
            src, "get_daily_average_discharge_from_iEH_HF_for_multiple_sites"
        ) as mock_api_func:
            result = src._fill_gaps_from_api(
                ieh_hf_sdk=mock_sdk,
                gaps=detected_gaps,
    ...
    def test_maintenance_mode_multiple_sites_independent_gaps(self, test_env):
        ...
        assert len(detected_gaps) == 2, (
            f"Expected 2 gaps (one per site), found {len(detected_gaps)}"
        )
...
class TestEdgeCases:
    ...
    def test_gap_boundary_at_cutoff_date(self):
        ...
        if gaps:
            ...
            assert gap_start >= cutoff, f"Gap start {gap_start} should be >= cutoff {cutoff}"
            # Gap should end at recent_date - 1 day
            expected_gap_end = recent_date - pd.Timedelta(days=1)
            assert gap_end == expected_gap_end, f"Gap end {gap_end} should be {expected_gap_end}"
    ...
    def test_manual_site_filtering_all_filtered_out(self):
        ...
        with patch.object(
            ...
            assert not mock_api_func.called, (
                "API should NOT be called when all gaps are filtered out"
            )
...
class TestCSVRoundTrip:
    ...
    def test_gap_detection_with_mixed_date_types(self):
        ...
        gaps = src._detect_gaps_in_data(
            data_with_string_dates, "code", "date", max_gap_age_days=3650
        )
    ...
    def test_type_consistency_in_gap_detection(self):
        ...
        assert type(sample_all) is type(sample_existing), (
