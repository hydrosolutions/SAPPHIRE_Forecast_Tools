...
class TestHydrographMultipleStations:
    ...
    def test_hydrograph_multiple_stations_separate_stats(self):
        ...
        current_year = dt.date.today().year
        df1 = create_test_timeseries(
            f"{current_year}-01-01", f"{current_year}-03-31", "99901", 100.0
        )
        df2 = create_test_timeseries(
            f"{current_year}-01-01", f"{current_year}-03-31", "99902", 200.0
        )
