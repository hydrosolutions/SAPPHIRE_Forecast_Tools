...
def test_data_retrieval(sdk, site_code, days=10):
    ...
    print(f"\n{'=' * 60}")
    print("TEST: Single Site Data Retrieval")
    print(f"{'=' * 60}")
    print(f"  Gauge: {site_code}")
    print(f"  Period: Last {days} days")
    print("  Filter: station__station_code__in (testing alternative to site_codes)")
...
def check_all_sites_for_recent_data(sdk, max_age_days=5):
    ...
    print(f"\n{'=' * 60}")
    print("TEST: Check All Sites for Recent Data")
    print(f"{'=' * 60}")
    print("  Filter: station__station_code__in (testing alternative to site_codes)")
    ...
    print(
        f"  Unique site codes: {len(site_codes)} (removed {len(site_codes_raw) - len(site_codes)} duplicates)"
    )

    # Query all sites at once
    # Using station__station_code__in instead of site_codes
    print(f"\nQuerying API for all {len(site_codes)} sites at once...")
    print("  (This may take a moment depending on data volume)")
    filters = {
...
def diagnose_422_error(sdk, site_ids):
    ...
    """
    print(f"\n{'=' * 60}")
    print("TEST: Diagnose 422 Errors (Individual Site Testing)")
    print(f"{'=' * 60}")
    print("  Filter: station__station_code__in (testing alternative to site_codes)")
                ...
                for r in response["results"]:
                    for d in r.get("data", []):
                        for v in d.get("values", []):
                            ts = (
                                v.get("timestamp_local", "")[:10]
                                if v.get("timestamp_local")
                                else None
                            )
...
def test_filter_comparison(sdk, site_codes, start_datetime, end_datetime):
    ...
    print(
        f"  Date range: {start_datetime.strftime('%Y-%m-%d')} to {end_datetime.strftime('%Y-%m-%d')}"
    )
    ...
    if success_a:
        ...
        print("  FAILED to fetch with site_codes filter")
    ...
    if success_b:
        ...
        print("  FAILED to fetch with station__station_code__in filter")
    ...
    if count_a == count_b and len(results_a) == len(results_b):
        ...
        print("  Both return the same number of records.")
    else:
...
def test_bulk_vs_individual(sdk, site_ids):
    ...
    """
    print(f"\n{'=' * 70}")
    print("TEST: Data Retrieval with Pagination")
    print(f"{'=' * 70}")
    print("  Filter: station__station_code__in (testing alternative to site_codes)")
    print(f"  Sites to test: {len(site_ids)}")
    print("\n  IMPORTANT: API has undocumented page_size limit of 10.")
    print("  This test verifies we can get ALL data by paging through results.")
    ...
    print("\n" + "-" * 70)
    print("Step 1/4: Fetching site code mapping from API...")
    print("-" * 70)
    discharge_sites = sdk.get_discharge_sites()
    id_to_code = {s.get("id"): s.get("site_code", "Unknown") for s in discharge_sites}
    ...
    print("\n" + "-" * 70)
    print("Step 2/4: Test pagination with a SINGLE site")
    print("-" * 70)
    ...
    if test_site:
        ...
        if success:
            ...
            print(
                f"  PAGINATION TEST: {'PASSED' if len(results) == total else 'FAILED'} (got {len(results)}/{total})"
            )
        else:
    ...
    print("-" * 70)
    print("  This will page through all results (page_size=10)...")
    ...
    if not success:
        print("  FAILED: Could not fetch data with station__station_code__in filter")
        print("  This might mean the filter parameter is not supported.")
        return [], []
    ...
    print("\n" + "-" * 70)
    print("Step 4/4: RESULTS ANALYSIS")
    print("-" * 70)
    results = all_results
    ...
    for _site_id, info in sorted(
        sites_with_data.items(), key=lambda x: x[1]["latest"] or "", reverse=True
    ):
        print(
            f"{info['code']:<10} {info['values']:<8} {info['latest'] or 'N/A':<12} {info['name']}"
        )
    ...
    if sites_without_data:
        ...
        print(
            f"  Querying {len(unique_sites_without_data_codes)} sites for WDD variable (with pagination)..."
        )
        wdd_filters = {
    ...
    print("SUMMARY:")
    print(f"{'=' * 70}")
    print("  Filter used:   station__station_code__in")
        ...
        if site_ids_str:
            ...
            if discharge_sites:
                ...
                if len(codes) != len(unique_codes):
                    ...
                    duplicates = [
                        (code, count) for code, count in Counter(codes).items() if count > 1
                    ]
