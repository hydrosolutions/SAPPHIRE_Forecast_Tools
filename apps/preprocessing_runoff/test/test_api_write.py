...
class TestWriteHydrographToApi:
    ...
    def test_mode_parameter_overrides_env_var(self, hydrograph_data):
        ...
        with (
            ...
            # Env says operational, but mode param says initial → all data
            result = src._write_hydrograph_to_api(hydrograph_data, mode="initial")
            assert result is True
            records = mock_client.write_hydrograph.call_args[0][0]
            assert len(records) == 80  # all 40 days × 2 stations
...
class TestPublicWriteFunctionsApiGraceful:
    ...
    def test_csv_write_succeeds_despite_api_error(self, runoff_data, tmp_path):
        ...
        with (
            patch.object(src, "SAPPHIRE_API_AVAILABLE", True),
            patch.object(src, "_write_runoff_to_api", side_effect=Exception("API down")),
            patch.dict(
                os.environ,
                {
    ...
    def test_hydrograph_csv_write_succeeds_despite_api_error(self, hydrograph_data, tmp_path):
        ...
        with (
            patch.object(src, "SAPPHIRE_API_AVAILABLE", True),
            patch.object(src, "_write_hydrograph_to_api", side_effect=Exception("API down")),
            patch.dict(
                os.environ,
                {
                    "ieasyforecast_intermediate_data_path": str(tmp_path),
    ...
    def test_mode_passed_through_to_runoff_api(self, runoff_data, tmp_path):
        ...
        with (
            patch.object(src, "SAPPHIRE_API_AVAILABLE", True),
            patch.object(src, "_write_runoff_to_api", return_value=True) as mock_api,
            patch.dict(
                os.environ,
                {
                    "ieasyforecast_intermediate_data_path": str(tmp_path),
    ...
    def test_mode_passed_through_to_hydrograph_api(self, hydrograph_data, tmp_path):
        ...
        with (
            patch.object(src, "SAPPHIRE_API_AVAILABLE", True),
            patch.object(src, "_write_hydrograph_to_api", return_value=True) as mock_api,
            patch.dict(
                os.environ,
                {
                    "ieasyforecast_intermediate_data_path": str(tmp_path),
