...
class TestMergeWithUpdate:
    ...
    def test_merge_update_newer_timestamp(self):
        ...
        result = src._merge_with_update(cached_data, new_data, "code", "date", "discharge")
    ...
    def test_merge_empty_cached_data(self):
        ...
        assert len(result) == len(new_data), f"Expected {len(new_data)} rows, got {len(result)}"

        # Verify all new data is present
        for _, row in new_data.iterrows():
            matching = result[(result["code"] == row["code"]) & (result["date"] == row["date"])]
            assert len(matching) == 1, f"Missing row: code={row['code']}, date={row['date']}"
