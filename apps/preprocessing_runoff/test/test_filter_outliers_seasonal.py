...
class TestSeasonalDataPreservation:
    ...
    def test_winter_only_data_preserved(self):
        ...
        df = pd.DataFrame(
            {"Code": 16059, "date": dates, "Q_m3s": np.random.uniform(100, 200, len(dates))}
        )
    ...
    def test_multiple_stations_seasonal_data_preserved(self):
        ...
        for code in stations:
            ...
            dfs.append(
                pd.DataFrame(
                    {"Code": code, "date": dates, "Q_m3s": np.random.uniform(100, 200, len(dates))}
                )
            )
...
class TestReindexBehavior:
    ...
    def test_interpolation_fills_small_gaps(self):
        ...
        jan3 = result[(result["Code"] == 16059) & (result["date"] == "2020-01-03")]["Q_m3s"].values[
            0
        ]
        jan4 = result[(result["Code"] == 16059) & (result["date"] == "2020-01-04")]["Q_m3s"].values[
            0
        ]
    ...
    def test_large_gaps_remain_nan(self):
        ...
        jan7 = result[(result["Code"] == 16059) & (result["date"] == "2020-01-07")]["Q_m3s"].values[
            0
        ]
...
class TestIQRFiltering:
    ...
    def test_iqr_uses_seasonal_statistics(self):
        ...
        summer_result = result[
            (result["Code"] == 16059) & (result["date"].dt.month.isin([6, 7, 8]))
        ]
...
class TestEdgeCases:
    ...
    def test_single_day_data(self):
        ...
        df = pd.DataFrame(
            {"Code": [16059], "date": pd.to_datetime(["2020-06-15"]), "Q_m3s": [150.0]}
        )
    ...
    def test_nan_in_input_preserved(self):
        ...
        df = pd.DataFrame({"Code": 16059, "date": dates, "Q_m3s": values})

        result = src.filter_roughly_for_outliers(df)

        # The NaN on day 3 should be interpolated (gap of 1 day)
        day3 = result[(result["Code"] == 16059) & (result["date"] == "2020-01-03")]["Q_m3s"].values[
            0
        ]
...
class TestRegressionBug:
    ...
    def test_regression_november_data_not_lost(self):
        ...
        nov_with_data = (
            result[(result["Code"] == 16059) & (result["date"].dt.month == 11)]["Q_m3s"]
            .notna()
            .sum()
        )
    ...
    def test_regression_all_seasons_have_data_when_input_has_data(self):
        ...
        for month in range(1, 13):
            month_with_data = (
                result[(result["Code"] == 16059) & (result["date"].dt.month == month)]["Q_m3s"]
                .notna()
                .sum()
            )
