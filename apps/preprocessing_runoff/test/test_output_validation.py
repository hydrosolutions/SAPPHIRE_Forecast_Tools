...
class TestHydrographDayCSVSchema:
        ...
        df = pd.DataFrame(
            {
                ...
                "discharge": np.random.randn(366 if src.is_leap_year(current_year) else 365) * 10
                + 100,
    ...
    def test_hydrograph_day_no_negative_stats(self):
        ...
        df = pd.DataFrame(
            {
                "date": pd.date_range(
                    f"{current_year - 2}-01-01", f"{current_year}-03-31", freq="D"
                ),
                "code": ["99901"]
                * len(
                    pd.date_range(f"{current_year - 2}-01-01", f"{current_year}-03-31", freq="D")
                ),
                "discharge": np.abs(
                    np.random.randn(
                        len(
