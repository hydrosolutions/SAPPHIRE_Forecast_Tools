...
def get_ieh_sdk():
    ...
    if os.getenv("ieasyhydroforecast_connect_to_iEH") == "True":
        ...
        if (
            os.getenv("ieasyhydroforecast_organization") == "demo"
            or os.getenv("ieasyhydroforecast_organization") == "tjhm"
        ):
...
def test_data_retrieval(org, connect_to_ieh, expected_success):
    ...
    try:
        if connect_to_ieh:
            ...
            if success:
                fc_sites_pentad, site_codes_pentad = sl.get_pentadal_forecast_sites(
                    ieh_sdk, backend_has_access_to_db=has_access_to_db
                )
                ...
                logger.info(
                    "Skipping site code comparison because iEH access has failed as expected."
                )
            ...
            if success:
                fc_sites_pentad, site_codes_pentad, site_ids_pentad = (
                ...
                logger.info(
                    "Skipping site code comparison because iEH HF access has failed as expected."
                )
...
def test_validation_of_env_variables(org, expected_success):
    ...
    try:
        ...
        if os.getenv("ieasyhydroforecast_organization") != org:
            logger.error(
                f"Expected organization: {org}, but got: {os.getenv('ieasyhydroforecast_organization')}"
            )
            return False
        ...
        if os.getenv("ieasyhydroforecast_connect_to_iEH") == "True" and expected_success:
            ...
            logger.info(
                "Environment variables validation passed, but connection to iEH is not expected."
            )
            return True
...
if __name__ == "__main__":
    ...
    test_env_paths = {
        "kghm": os.path.expanduser(
            "~/Documents/GitHub/sensitive_data_forecast_tools/config/.env_develop_kghm"
        ),
        "tjhm": os.path.expanduser(
            "~/Documents/GitHub/taj_data_forecast_tools/config/.env_develop_tjhm"
        ),
    ...
    for org, expected_success in test_organizations.items():
        ...
        original_ieasyforecast_daily_discharge_path = os.getenv(
        ...
        if original_ieasyforecast_configuration_path:
            os.environ["ieasyforecast_configuration_path"] = os.path.join(
                script_dir, "..", original_ieasyforecast_configuration_path
            )
        if original_ieasyforecast_daily_discharge_path:
            os.environ["ieasyforecast_daily_discharge_path"] = os.path.join(
                script_dir, "..", original_ieasyforecast_daily_discharge_path
            )

        # Run the test for iEH
        logger.info("== Test connecting to iEH ==")
        ...
        logger.info("== Test connecting to iEH HF ==")
