...
def _check_snow_consistency(
    ...
    if is_consistent:
        ...
        print(
            f"CONSISTENCY_CHECK: FAILED ({snow_type}, HRU {hru_code}) "
            "- see log for details"
        )
...
def get_snow_data_operational(client, hru, variable, date, dg_path,
                              save_path):
    """
    ...
    """
    logger.info(
        "Processing snow data: HRU %s, variable %s", hru, variable
    )
    ...
    try:
        ...
        logger.error(
            "Error getting snow data from Data Gateway for "
...
def main():
    ...
    for hru in SNOW_HRUS:
        for snow_var in SNOW_VARS:
            ...
            if not success:
                logger.error(
                    "Failed to get snow data for HRU %s, %s",
                    hru, snow_var
                )
