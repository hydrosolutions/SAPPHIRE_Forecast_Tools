...
def _check_snow_consistency(
    ...
    if csv_recent.empty:
        logger.info(
            "%s: No recent data to check consistency", tag
        )
        return True
    ...
    logger.info(
        ...
        tag, min_date.date(), max_date.date(), list(codes), api_url
    )
    if csv_val_col:
        for code in codes:
            ...
            logger.info(
                "%s: CSV values for code=%s (%d values, "
                "last 5): %s",
                tag, code, len(csv_vals), csv_vals[-5:]
            )
    ...
    for code in codes:
        try:
            ...
            if api_df.empty:
                logger.warning(
                    "%s: API returned 0 rows for code=%s",
                    tag, code
                )
            else:
    ...
    if not all_api_data:
        ...
        try:
            any_data = client.read_snow(
                snow_type=snow_type.upper(), limit=5
            )
            if any_data.empty:
                logger.warning(
                    "%s: FAILED - API has NO %s data at all. "
                ...
                logger.warning(
                    ...
                    "has %s data for other dates: %s",
                    tag, min_date.date(), max_date.date(),
                    snow_type.upper(), dates_in_api[:5]
                )
        except Exception as e:
            logger.warning(
                "%s: FAILED - Diagnostic read also failed: %s",
                tag, e
            )
        return False
    ...
    if is_consistent:
        ...
        print(
            ...
            f"({snow_type}, HRU {hru_code}) - see log for details"
        )

    return is_consistent


def get_snow_data_reanalysis(client, hru, variable, start_date,
                             end_date, dg_path, save_path):
    """
    ...
    """
    logger.info(
        "Processing snow reanalysis: HRU %s, %s (%s to %s)",
        hru, variable, start_date, end_date
    )
    ...
    if os.path.exists(file_path):
        try:
            ...
            old_dataframe = old_dataframe.sort_values(
                by=['date', 'code']
            )
            logger.info(
    ...
    try:
        outpath = client.get_snow_reanalysis(
            ...
            parameter=variable, directory=dg_path
        )
    except Exception as e:
        logger.error(
            "Error getting reanalysis data from Data Gateway for "
            "HRU %s, %s: %s", hru, variable, e
        )
        return False
    ...
    df_combined = pd.concat(
        [old_dataframe, df_transformed], ignore_index=True
    )
    ...
    try:
        ...
        logger.error(
            "Error writing snow data to API (HRU %s, %s): %s",
            hru, variable, e
        )
...
def main():
    ...
    while this_start <= year_end:
        ...
        date_intervals.append(
            (f"{this_start}-01-01", f"{this_end}-01-01")
        )
        this_start = this_end

    logger.info(
        "Processing %d date batches x %d HRUs x %d variables",
        len(date_intervals), len(SNOW_HRUS), len(SNOW_VARS)
    )

    total = len(date_intervals) * len(SNOW_HRUS) * len(SNOW_VARS)
    count = 0
    for batch_idx, (start_date, end_date) in enumerate(
        date_intervals, 1
    ):
        logger.info(
            "=== Batch %d/%d: %s to %s ===",
            batch_idx, len(date_intervals), start_date, end_date
        )
        for hru in SNOW_HRUS:
            for snow_var in SNOW_VARS:
                ...
                if not success:
                    logger.error(
                        "Failed to get reanalysis data for "
                        "HRU %s, %s", hru, snow_var
                    )

    logger.info(
        "Snow reanalysis processing complete (%d tasks)", total
    )
