...
class TestSelectStableOperationalData:
    ...
    def test_default_195_day_threshold(self):
        ...
        df = pd.DataFrame({
            'date': pd.to_datetime([
                '2023-01-01', '2023-12-18', '2023-12-19', '2024-07-01'
            ]),
            'code': ['A'] * 4,
            'P': [1.0, 2.0, 3.0, 4.0],
        })
        ...
        assert len(result) == 2
        expected_dates = {
            pd.Timestamp('2023-01-01'), pd.Timestamp('2023-12-18')
        }
    ...
    def test_custom_stability_days(self):
        ...
        df = pd.DataFrame({
            'date': pd.to_datetime([
                '2024-05-31', '2024-06-01', '2024-07-01'
            ]),
            'code': ['A'] * 3,
            'P': [1.0, 2.0, 3.0],
        })
...
class TestExtendReanalysisWithOperational:
    ...
    def test_appends_non_overlapping(self):
        ...
        result = extend_reanalysis_with_operational(
    ...
    def test_sorted_by_date_code(self):
        ...
        expected = result.sort_values(['date', 'code']).reset_index(
            drop=True
        )
    ...
    def test_empty_operational_returns_reanalysis(self):
        ...
        operational = pd.DataFrame(
            columns=['date', 'code', 'P']
        ).astype({'date': 'datetime64[ns]', 'P': 'float64'})
        result = extend_reanalysis_with_operational(
            reanalysis, operational
        )
        pd.testing.assert_frame_equal(
...
class TestCalculateDailyNorm:
    ...
    def test_norm_is_mean_across_years(self, three_year_reanalysis):
        ...
        result = calculate_daily_norm(
            three_year_reanalysis, operational, 'P', 2023
        )
    ...
    def test_leap_year_produces_366_rows_per_code(
        self, leap_year_reanalysis
    ):
        ...
        result = calculate_daily_norm(
            leap_year_reanalysis, operational, 'P', 2024
        )
    ...
    def test_non_leap_year_produces_365_rows_per_code(
        self, leap_year_reanalysis
    ):
        ...
        result = calculate_daily_norm(
            leap_year_reanalysis, operational, 'P', 2023
        )
    ...
    def test_non_leap_year_day365_present_day366_absent(self):
        ...
        for code in ['A', 'B']:
            ...
            dec31 = code_rows[
                code_rows['date'] == pd.Timestamp('2023-12-31')
            ]
            assert len(dec31) == 1
            # No Jan 1 next year (which day 366 would produce)
            jan1_next = code_rows[
                code_rows['date'] == pd.Timestamp('2024-01-01')
            ]
    ...
    def test_non_leap_year_no_historical_leaps(self):
        ...
        # Dec 31 present
        dec31 = result[
            result['date'] == pd.Timestamp('2023-12-31')
        ]
        assert len(dec31) == 1
    ...
    def test_multiple_codes_independent_norms(self):
        ...
        frames = []
        for year, a_val, b_val in [(2021, 10.0, 100.0),
                                    (2022, 20.0, 200.0)]:
