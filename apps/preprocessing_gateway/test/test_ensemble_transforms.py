...
class TestTransformDataFileEnsembleMember:
    ...
    def dg_ensemble_dataframe(self):
        ...
        return pd.DataFrame(
            rows, columns=['Unnamed: 0', 'band_1000', 'band_2000']
        )
    ...
    def test_basic_columns_and_row_count(self, dg_ensemble_dataframe):
        ...
        result = transform_data_file_ensemble_member(
            dg_ensemble_dataframe, '12345'
        )
    ...
    def test_name_column_preserves_band_identifier(
        self, dg_ensemble_dataframe
    ):
...
class TestMergeEnsembleForecast:
    ...
    def ensemble_csv_factory(self, tmp_path):
        ...
        def _make(hru_code, ensemble_member, variable, dates, values,
                  band_name='band_1000'):
            value_type = 'P' if variable == 'tp' else 'T'
            filename = (
                f"prefix_EM{ensemble_member:03d}"
                f"_HRU{hru_code}_{variable}.csv"
            )
            ...
            data_rows = [[d, str(v)] for d, v in zip(dates, values)]
            all_rows = header_rows + data_rows
            df = pd.DataFrame(
                all_rows, columns=['Unnamed: 0', band_name]
            )
    ...
    def test_filename_parsing_hru_code(self, ensemble_csv_factory):
        ...
        p_file = ensemble_csv_factory(
            '12345', 1, 'tp', ['01/01/2024'], [5.0]
        )
        t_file = ensemble_csv_factory(
            '12345', 1, '2t', ['01/01/2024'], [280.0]
        )
        result = merge_ensemble_forecast([p_file, t_file])
        assert result['code'].iloc[0] == '12345'

    def test_filename_parsing_ensemble_member(
        self, ensemble_csv_factory
    ):
        """EM003 → ensemble_member == 3 in output."""
        p_file = ensemble_csv_factory(
            '12345', 3, 'tp', ['01/01/2024'], [5.0]
        )
        t_file = ensemble_csv_factory(
            '12345', 3, '2t', ['01/01/2024'], [280.0]
        )
        result = merge_ensemble_forecast([p_file, t_file])
    ...
    def test_merge_produces_both_p_and_t_columns(
        self, ensemble_csv_factory
    ):
    ...
    def test_outer_merge_preserves_unmatched(
        self, ensemble_csv_factory
    ):
        ...
        p_file = ensemble_csv_factory(
            '12345', 1, 'tp', p_dates, [5.0, 6.0, 7.0]
        )
        t_file = ensemble_csv_factory(
            '12345', 1, '2t', t_dates, [280.0, 281.0]
        )
        result = merge_ensemble_forecast([p_file, t_file])
        assert len(result) == 3
        result_sorted = result.sort_values('date').reset_index(drop=True)
    ...
    def test_multiple_ensemble_members_merged(
        self, ensemble_csv_factory
    ):
    ...
    def test_unrecognized_variable_skipped(
        self, ensemble_csv_factory, tmp_path
    ):
        ...
        result = merge_ensemble_forecast(
            [p_file, t_file, str(rh_path)]
        )
        assert 'P' in result.columns
        assert 'T' in result.columns
        assert len(result) == 1
