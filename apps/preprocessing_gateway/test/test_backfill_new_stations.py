...
class TestDetectMeteoGaps:
    ...
    def test_new_station_not_in_api(self):
        ...
        new, stale = bns.detect_meteo_gaps(csv_codes, api_coverage, "T")
        assert new == {"ST002"}
        assert stale == set()
    ...
    def test_stale_station(self):
        ...
        new, stale = bns.detect_meteo_gaps(csv_codes, api_coverage, "T", staleness_days=7)
        assert new == set()
        assert stale == {"ST001"}
    ...
    def test_multiple_new_and_stale(self):
        ...
        new, stale = bns.detect_meteo_gaps(csv_codes, api_coverage, "P", staleness_days=7)
        assert new == {"C"}
        assert stale == {"B", "D"}
...
class TestDetectSnowGaps:
    ...
    def test_new_snow_station(self):
        ...
        new, stale = bns.detect_snow_gaps(csv_codes, api_coverage, "SWE")
        assert new == {"HRU01"}
        assert stale == set()
    ...
    def test_stale_snow_station(self):
        ...
        new, stale = bns.detect_snow_gaps(csv_codes, api_coverage, "SWE", staleness_days=7)
        assert stale == {"HRU01"}
    ...
    def test_snow_type_case_handling(self):
        ...
        new, stale = bns.detect_snow_gaps(csv_codes, api_coverage, "hs", staleness_days=7)
        assert stale == {"HRU01"}
