...
class TestErrorPropagation:
    ...
    def test_api_write_failure_does_not_block_csv(self, gateway_env, mock_api_client):
    ...
    def test_api_unavailable_skips_gracefully(self, gateway_env, mock_api_client):
    ...
    def test_missing_reanalysis_csv_handled(self, gateway_env):
        ...
        with (
            patch("extend_era5_reanalysis.sl.load_environment"),
...
class TestBackfillWorkflow:
    ...
    def test_backfill_detects_new_station_and_writes(self, gateway_env, mock_api_client):
        ...
        with (
            patch("backfill_new_stations.sl.load_environment"),
    ...
    def test_backfill_detects_stale_and_writes_incremental(self, gateway_env, mock_api_client):
        ...
        codes_written = {r["code"] for r in all_meteo}
        assert "12345" in codes_written
        assert "67890" not in codes_written, "Up-to-date code should not be backfilled"

        # All written dates should be after the stale_date
        for record in all_meteo:
            record_date = date.fromisoformat(record["date"])
            assert record_date > stale_date, (
                f"Record date {record_date} should be after stale cutoff {stale_date}"
            )

    def test_backfill_skips_up_to_date_stations(self, gateway_env, mock_api_client):
...
class TestCrossScriptDataFlow:
    ...
    def test_qm_output_consumed_by_extend_era5(self, gateway_env, mock_api_client):
        """Run QM → check output → run extend_era5 → verify extension."""
        env = gateway_env
        self._run_qm(env, env["codes"])

        # Read reanalysis before extend
        reanalysis_p = pd.read_csv(env["reanalysis_dir"] / "HRU001_P_reanalysis.csv")
        ...
        reanalysis_p_after = pd.read_csv(env["reanalysis_dir"] / "HRU001_P_reanalysis.csv")
        # Should have more rows (stable operational appended)
        assert len(reanalysis_p_after) >= rows_before, (
            f"Reanalysis should grow: {rows_before} → {len(reanalysis_p_after)}"
        )

    def test_extend_era5_appends_to_existing_reanalysis(self, gateway_env):
        """Reanalysis CSV grows by the new dates from QM output."""
        env = gateway_env
        self._run_qm(env, env["codes"])

        reanalysis_before = pd.read_csv(env["reanalysis_dir"] / "HRU001_P_reanalysis.csv")
        ...
        reanalysis_after = pd.read_csv(env["reanalysis_dir"] / "HRU001_P_reanalysis.csv")
    ...
    def test_norm_calculation_uses_extended_data(self, gateway_env):
        ...
        dashboard_p = env["reanalysis_dir"] / "HRU001_P_reanalysis_dashboard.csv"
        ...
        assert non_null_norms > 300, f"Expected >300 non-null norms, got {non_null_norms}"

        # Spot-check: norm values should be reasonable (positive for P)
        assert df["P_norm"].dropna().min() >= 0

    def test_full_three_script_pipeline(self, gateway_env, mock_api_client):
        ...
        snow_dates = [
            (datetime.today() - timedelta(days=d)).strftime("%d.%m.%Y") for d in range(30, -1, -1)
        ]
        snow_vals = [[50.0] for _ in snow_dates]
        snow_df = make_dg_snow_csv(env["snow_codes"], snow_dates, snow_vals)
        ...
        with (
            patch("snow_data_operational.sl.load_environment"),
            patch(
                "snow_data_operational.snow_model.SapphireSnowModelClient",
        ...
        assert (env["reanalysis_dir"] / "HRU001_P_reanalysis.csv").exists()

        # Dashboard CSVs exist
        assert (env["reanalysis_dir"] / "HRU001_P_reanalysis_dashboard.csv").exists()
        assert (env["reanalysis_dir"] / "HRU001_T_reanalysis_dashboard.csv").exists()

        # Snow CSVs exist
        snow_swe = env["swe_dir"] / "HRU_SNOW01_SWE.csv"
        snow_hs = env["hs_dir"] / "HRU_SNOW01_HS.csv"
        assert snow_swe.exists() or snow_hs.exists(), "At least one snow CSV should exist"
...
class TestSyncModes:
    ...
    def test_operational_mode_writes_recent_days(self, gateway_env, mock_api_client):
        """Operational: meteo API records have yesterday and/or today."""
        env = gateway_env
        self._run_qm_pipeline(env, env["codes"], mock_api_client, "operational")
        ...
        if len(all_meteo) > 0:
            ...
            assert dates_written <= allowed, (
                f"Operational should write only yesterday+today ({allowed}), got {dates_written}"
            )

    def test_maintenance_mode_writes_recent_history(self, gateway_env, mock_api_client):
        """Maintenance: meteo API records span ~30 days."""
        env = gateway_env
        self._run_qm_pipeline(env, env["codes"], mock_api_client, "maintenance")
        ...
        date_range = date.fromisoformat(dates[-1]) - date.fromisoformat(dates[0])
        # Should span at least a few days, up to ~30
        assert date_range.days >= 1, f"Maintenance should span multiple days, got {date_range.days}"
        assert date_range.days <= 31, (
            f"Maintenance should not exceed 31 days, got {date_range.days}"
        )

    def test_initial_mode_writes_all_data(self, gateway_env, mock_api_client):
        """Initial: meteo API records include full CSV history."""
        env = gateway_env
        self._run_qm_pipeline(env, env["codes"], mock_api_client, "initial")
    ...
    def test_reanalysis_api_write_skipped_in_operational(self, gateway_env, mock_api_client):
        ...
        assert spy.call_count >= 1, "Expected _write_reanalysis_to_api to be called"
        for _call_result in [spy.return_value]:
    ...
    def test_reanalysis_api_write_active_in_maintenance(self, gateway_env, mock_api_client):
        ...
        assert len(all_meteo) > 0, "Maintenance mode should write meteo records"
...
class TestSnowPipelineIntegration:
    ...
    def _make_snow_env_and_client(self, env, snow_codes, dates, values, var_name="SWE", n_bands=0):
        """Create snow DG CSV and mock client."""
        snow_df = make_dg_snow_csv(snow_codes, dates, values, var_name, n_bands)
    ...
    def test_snow_download_transform_write_csv(self, gateway_env, mock_api_client):
        """Snow client → transform → CSV written with correct columns."""
        env = gateway_env
        today = datetime.today()
        dates = [(today - timedelta(days=d)).strftime("%d.%m.%Y") for d in range(10, -1, -1)]
        values = [[50.0] for _ in dates]

        mock_snow = self._make_snow_env_and_client(env, env["snow_codes"], dates, values, "SWE")
    ...
    def test_snow_api_write_includes_elevation_bands(self, gateway_env, mock_api_client):
        ...
        if len(all_snow) > 0:
            ...
            has_bands = any(f"value{i}" in record for i in range(1, 4))
            assert has_bands, f"Expected elevation band values in record: {list(record.keys())}"
    ...
    def test_snow_multiple_hrus(self, gateway_env, mock_api_client):
        ...
        os.environ["ieasyhydroforecast_HRU_SNOW_DATA"] = "HRU_SNOW01,HRU_SNOW02"

        today = datetime.today()
        dates = [(today - timedelta(days=d)).strftime("%d.%m.%Y") for d in range(5, -1, -1)]
        values = [[50.0] for _ in dates]

        snow_df = make_dg_snow_csv(env["snow_codes"], dates, values, "SWE")
    ...
    def test_snow_operational_vs_maintenance_filtering(self, gateway_env, mock_api_client):
        ...
        mock_api_client.write_snow.side_effect = lambda recs: (
            mock_api_client._snow_writes.append(recs) or len(recs)
        )
