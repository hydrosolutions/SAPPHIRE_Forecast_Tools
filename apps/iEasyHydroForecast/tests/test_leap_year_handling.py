...
class TestLeapYearAlignment(unittest.TestCase):
    ...
    def test_day_of_year_nonleap_current_leap_last(self):
        ...
        current_is_leap = (current_year % 4 == 0 and current_year % 100 != 0) or (
            current_year % 400 == 0
        )
...
class TestDateReconstruction(unittest.TestCase):
    ...
    def test_old_day_of_year_method_fails_at_year_end(self):
        ...
        # Old method (buggy)
        old_result = pd.Timestamp(str(current_year)) + pd.to_timedelta(
            day_of_year_2024_dec31 - 1, unit="D"
        )
...
class TestIssueDateReconstruction(unittest.TestCase):
    ...
    def test_get_day_of_year_from_pentad(self):
        ...
        # pentad 1 = Jan 1-5, issue date = Dec 31 of PREVIOUS year
        self.assertEqual(
            get_day_of_year_from_pentad(1, 2025), 366
        )  # Dec 31, 2024 (leap year has 366 days)
        self.assertEqual(
            get_day_of_year_from_pentad(1, 2024), 365
        )  # Dec 31, 2023 (non-leap year has 365 days)
    ...
    def test_get_day_of_year_from_decad(self):
        ...
        # decad 1 = Jan 1-10, issue date = Dec 31 of PREVIOUS year
        self.assertEqual(
            get_day_of_year_from_decad(1, 2025), 366
        )  # Dec 31, 2024 (leap year has 366 days)
        self.assertEqual(
            get_day_of_year_from_decad(1, 2024), 365
        )  # Dec 31, 2023 (non-leap year has 365 days)
...
class TestIntegrationScenarios(unittest.TestCase):
    ...
    def test_scenario_2025_with_2024_data(self):
        ...
        has_feb29_after = ((data["date"].dt.month == 2) & (data["date"].dt.day == 29)).any()
...
class TestHydrographDataOutput(unittest.TestCase):
    ...
    def test_pentad_all_dates_and_day_of_year_filled(self):
        ...
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
        from forecast_library import get_day_of_year_from_pentad, get_issue_date_from_pentad
    ...
    def test_decad_all_dates_and_day_of_year_filled(self):
        ...
        sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))
        from forecast_library import get_day_of_year_from_decad, get_issue_date_from_decad
