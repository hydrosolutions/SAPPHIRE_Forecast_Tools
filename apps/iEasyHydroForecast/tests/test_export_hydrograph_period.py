"""Tests for the P2b LAPTOP-SIDE export wrapper

``bin/export_hydrograph_period_history.sh``.
...
_EXPORT_WRAPPER = _REPO_ROOT / "bin" / "export_hydrograph_period_history.sh"
_FIXTURE_DIR = (
    _REPO_ROOT
    / "apps"
    / "iEasyHydroForecast"
...
def _run_wrapper(
    ...
    *,
    ...
    ``_P2B_EXPORT_SKIP_LOCATION_GUARD=1`` testing-only env hook) so the
    ...
    if bypass_location_guard:
        full_env["_P2B_EXPORT_SKIP_LOCATION_GUARD"] = "1"
...
def test_export_wrapper_rejects_missing_pg_env(tmp_path):
    ...
    result = subprocess.run(
        ...
        env={
            ...
            "_P2B_EXPORT_SKIP_LOCATION_GUARD": "1",
        },
    )
    assert result.returncode != 0
    combined = (result.stdout + result.stderr).lower()
...
def test_pentad_sample_manifest_validates():
    ...
    csv = _FIXTURE_DIR / "pentad_sample.csv"
    result = _common.validate_manifest(csv, "hydrograph_period")
    assert result["export_type"] == "hydrograph_period"
