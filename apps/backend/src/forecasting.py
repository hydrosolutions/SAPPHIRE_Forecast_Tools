...
def get_predictor_pentad(modified_data, start_date, fc_sites, ieh_sdk, backend_has_access_to_db, predictor_dates):
    ...
    if backend_has_access_to_db and start_date.date() > dt.datetime.strptime('2020-01-01', '%Y-%m-%d').date():
        ...
        # For each site in fc_sites, get the predictor discharge from the DB
        for site in fc_sites:
            fl.Site.from_DB_get_predictor_sum(ieh_sdk, site, predictor_dates, lagdays=20)
        ...
        if '16936' in [site.code for site in fc_sites]:
            ...
            for site in tok_contrib_sites:
                ...
                tok_sites.append(tok_site)
            for site in tok_sites:
                fl.Site.from_DB_get_predictor_sum(ieh_sdk, site, predictor_dates)
...
def get_predictor_decad(modified_data, start_date, fc_sites, ieh_sdk, backend_has_access_to_db, predictor_dates):
    ...
    logger.info("Getting predictor for decadal forecasting ...")
    # region Calculate forecast
    # Get the sum of the last 3 days discharge from the database
    # We get the predictor discharge from the DB only if we are not in
    # operational mode or if the date is after 2020-01-01 in offline mode. This
    ...
    if backend_has_access_to_db and start_date.date() > dt.datetime.strptime('2020-01-01', '%Y-%m-%d').date():
        ...
        # For each site in fc_sites, get the predictor discharge from the DB
        for site in fc_sites:
            fl.Site.from_DB_get_predictor_mean(ieh_sdk, site, predictor_dates, lagdays=20)
        ...
        if '16936' in [site.code for site in fc_sites]:
            ...
            for site in tok_contrib_sites:
                ...
                tok_sites.append(tok_site)
            for site in tok_sites:
                fl.Site.from_DB_get_predictor_mean(ieh_sdk, site, predictor_dates)
    ...
    for site in fc_sites:
        if site.predictor is None or float(site.predictor) < 0.0 or start_date.date() < dt.datetime.strptime('2023-01-01', '%Y-%m-%d').date():
            ...
            #print("DEBUG: forecasting:get_predictor: predictor_dates: ", predictor_dates)
            # Get the predictor from the data for today
            fl.Site.from_df_get_predictor_decad(site, site_data1, [start_date])
            # print(fl.Site.from_df_get_predictor(site, site_data, predictor_dates))
            #print("DEBUG: forecasting:get_predictor: site.predictor: ", site.predictor)
...
def perform_forecast_pentad(fc_sites, forecast_pentad_of_year, result_df):
    # Perform forecast
    logger.info("Performing pentad forecast ...")

    # For each site, calculate the forecasted discharge
    for site in fc_sites:
        fl.Site.from_df_calculate_forecast_pentad(site, forecast_pentad_of_year, result_df)
...
def perform_forecast_decad(fc_sites, forecast_decad_of_year, result_df):
    # Perform forecast
    logger.info("Performing decad forecast ...")

    # For each site, calculate the forecasted discharge
    for site in fc_sites:
        fl.Site.from_df_calculate_forecast_decad(site, forecast_decad_of_year, result_df)
