...
def _make_long_forecast_records(code, year, months, model_type="GBT", q50=120.0):
...
class TestReadDailyRunoffApiIntegration:
    ...
    def test_multiple_codes_fetched_sequentially(self):
        ...
        with (
            ...
            result = _read_daily_runoff_api(["15013", "15020"], 2023, 2023)
...
class TestReadLongForecastsApiIntegration:
    ...
    def test_multiple_codes_combined(self):
        ...
        with (
            ...
            result = _read_long_forecasts_api(["15013", "15020"], 2023, 2023)
...
class TestObservationsPipelineIntegration:
    ...
    def test_full_pipeline_multi_station_multi_year(self):
        ...
        with patch("src.data_reader._read_daily_runoff_api") as mock:
            ...
            result = read_monthly_observations(["15013", "15020"], 2022, 2023)
        ...
        s13_2022 = result[(result["code"] == "15013") & (result["year"] == 2022)]
        assert s13_2022.iloc[0]["discharge_avg"] == pytest.approx(100.0)

        s20_2023 = result[(result["code"] == "15020") & (result["year"] == 2023)]
...
class TestForecastsPipelineIntegration:
    ...
    def test_full_pipeline_multi_model_multi_station(self):
        ...
        records = pd.concat(
            [
                _make_long_forecast_records("15013", 2023, [1, 2], model_type="GBT", q50=100.0),
                _make_long_forecast_records("15013", 2023, [1, 2], model_type="LR_Base", q50=90.0),
                _make_long_forecast_records("15020", 2023, [1, 2], model_type="GBT", q50=200.0),
            ],
            ignore_index=True,
        )
        ...
        with patch("src.data_reader._read_long_forecasts_api") as mock:
            ...
            result = read_monthly_forecasts(["15013", "15020"], 2023, 2023)
        ...
        gbt_15013_jan = result[
            (result["code"] == "15013") & (result["model_short"] == "GBT") & (result["month"] == 1)
        ]
    ...
    def test_pipeline_quantiles_preserved_end_to_end(self):
        ...
        q50 = 150.0
        records = _make_long_forecast_records("15013", 2023, [6], model_type="SM_GBT", q50=q50)
    ...
    def test_pipeline_year_month_extraction_multi_year(self):
        ...
        assert len(result) == 4
        assert set(result["year"]) == {2022, 2023}
        dec_row = result[(result["year"] == 2022) & (result["month"] == 12)]
        assert len(dec_row) == 1
        jan_row = result[(result["year"] == 2023) & (result["month"] == 1)]
...
class TestObservationsAndForecastsCombined:
    ...
    def test_observations_and_forecasts_can_merge_on_key(self):
        ...
        with (
            patch("src.data_reader._read_daily_runoff_api") as mock_obs,
    ...
    def test_merge_multiple_models_per_month(self):
        ...
        daily = _make_daily_runoff("15013", "2023-06-01", "2023-06-30", 100.0)

        fc_gbt = _make_long_forecast_records("15013", 2023, [6], model_type="GBT", q50=110.0)
        fc_lr = _make_long_forecast_records("15013", 2023, [6], model_type="LR_Base", q50=95.0)
    ...
    def test_no_overlap_produces_empty_merge(self):
        ...
        daily = _make_daily_runoff("15013", "2023-01-01", "2023-01-31", 100.0)
        forecasts_raw = _make_long_forecast_records("15013", 2023, [6], model_type="GBT", q50=110.0)
