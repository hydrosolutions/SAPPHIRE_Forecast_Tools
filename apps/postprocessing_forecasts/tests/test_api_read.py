...
def create_mock_ml_forecast_data():
    ...
    Station 12345: base=100, daily forecasted_discharge 101..110, mean=105.5
    Station 12346: base=150, daily forecasted_discharge 151..160, mean=155.5
    """
    rows = []
    for i, code in enumerate(["12345", "12346"]):
        base = 100.0 + i * 50  # 100 for 12345, 150 for 12346
...
class TestReadLRForecastsFromApi:
    ...
    def test_pagination_handling(self, mock_client_class):
        ...
        result = sl._read_lr_forecasts_from_api(
            horizon_type="pentad", site_codes=["12345", "67890"]
        )
    ...
    def test_site_codes_filter(self, mock_client_class):
        ...
        sl._read_lr_forecasts_from_api(horizon_type="pentad", site_codes=["12345", "67890"])
...
class TestMLAggregation:
    ...
    def test_pentad_aggregation_averages_quantiles(self, mock_client_class):
        ...
        Station 12345: daily forecasted_discharge = 101..110, mean = 105.5
        Station 12346: daily forecasted_discharge = 151..160, mean = 155.5
        """
        if not SAPPHIRE_API_AVAILABLE:
            pytest.skip("sapphire-api-client not installed")
    ...
    def test_aggregation_flag_takes_worst(self, mock_client_class):
        ...
        mask = (mock_data["code"] == "12345") & (mock_data["target"] == pd.Timestamp("2024-01-03"))
    ...
    def test_api_metadata_columns_not_in_output(self, mock_client_class):
        ...
        for col in ["id", "model_type", "horizon_type", "model_type_description", "composition"]:
            assert col not in result.columns, f"API metadata column '{col}' leaked into output"
    ...
    def test_output_has_required_downstream_columns(self, mock_client_class):
        ...
        for col in required_cols:
            assert col in result.columns, f"Required column '{col}' missing from API reader output"
    ...
    def test_different_model_types(self, mock_save, mock_api_read, temp_csv_env):
        ...
        try:
            for model in ["TIDE", "TSMIXER", "ARIMA"]:
                ...
                sl.read_machine_learning_forecasts_pentad(model=model)
