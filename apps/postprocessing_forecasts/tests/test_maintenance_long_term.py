...
def _make_gaps(tuples):
    ...
    return pd.DataFrame(
        tuples, columns=['year', 'month', 'code', 'model_short'],
    )
...
def forecasts_for_gaps():
    ...
    return _make_forecasts([
        {'year': 2025, 'month': 1, 'code': '10001',
         'model_short': 'LR', 'q50': 100.0, 'q05': 80.0,
         'q95': 120.0, 'valid_from': '2025-01-01',
         'valid_to': '2025-01-31', 'date': '2025-01-01', 'flag': 0},
        {'year': 2025, 'month': 1, 'code': '10001',
         'model_short': 'TFT', 'q50': 110.0, 'q05': 85.0,
         'q95': 130.0, 'valid_from': '2025-01-01',
         'valid_to': '2025-01-31', 'date': '2025-01-01', 'flag': 0},
    ])
...
def ensemble_result(forecasts_for_gaps):
    ...
    em_row = pd.DataFrame([{
        ...
        'month_in_year': 1, 'q50': 105.0, 'q05': 82.5,
        'q95': 125.0, 'valid_from': '2025-01-01',
        'valid_to': '2025-01-31', 'date': '2025-01-01', 'flag': 0,
    }])
    return pd.concat(
        [forecasts_for_gaps, em_row], ignore_index=True,
    )
...
class TestMaintenanceLongTerm:
    ...
    def test_no_gaps_found_exits_zero(self, combined_with_models):
        ...
        mock_gap_detector.detect_missing_monthly_ensembles.return_value = (
            pd.DataFrame(columns=['year', 'month', 'code', 'model_short'])
        )
        ...
        with patch.dict(sys.modules, {}):
            ...
            call_args = (
                mock_gap_detector
                .detect_missing_monthly_ensembles.call_args
            )
            pd.testing.assert_frame_equal(
                call_args[0][0], combined_with_models,
            )
            assert call_args[0][1] == 3  # default lookback
            assert call_args[1]['ensemble_models'] == {
                'EM', 'Skilled Mean', 'Naive Mean',
            }
    ...
    def test_gaps_found_creates_and_saves_ensembles(
        ...
        mock_data_reader.read_monthly_forecasts.return_value = (
            forecasts_for_gaps
        )
        mock_ensemble_calc.create_monthly_ensemble_forecasts.return_value = (
            ensemble_result
        )
        ...
        with patch.dict(sys.modules, {}):
            ...
            saved_df = (
                mock_file_writer.save_monthly_forecast_data.call_args[0][0]
            )
    ...
    def test_deduplication_works(
        ...
        combined = _make_combined([
            {'year': 2025, 'month': 1, 'code': '10001',
             ...
             'month_in_year': 1},
            {'year': 2025, 'month': 1, 'code': '10001',
             'model_short': 'EM', 'forecasted_discharge': 99.0,
             'month_in_year': 1},
        ])
        ...
        ensemble_output = pd.concat(
            [forecasts_for_gaps, new_em], ignore_index=True,
        )
        ...
        with patch.dict(sys.modules, {}):
            ...
            assert len(em_rows) == 1, (
                f"Expected exactly 1 EM row after dedup, got {len(em_rows)}"
            )
            assert em_rows.iloc[0]['forecasted_discharge'] == 105.0, (
    ...
    def test_audit_trail_logged(
        ...
        with patch.dict(sys.modules, {}):
            ...
            assert len(audit_msgs) >= 1, (
                "Expected at least one AUDIT log entry"
            )
            ...
            detail_msgs = [
                m for m in messages
                if 'Filled: year=2025, month=1, code=10001' in m
            ]
            assert len(detail_msgs) == 1, (
                "Expected one detail line per gap tuple"
            )
    ...
    def test_lookback_env_var_respected(self, combined_with_models):
        ...
        with patch.dict(
            os.environ, {'POSTPROCESSING_GAPFILL_WINDOW_MONTHS': '6'}
        ):
            with patch.dict(sys.modules, {}):
                module = _import_module({
                    'sl': mock_sl,
                ...
                assert call_args[0][1] == 6, (
                    f"Expected lookback=6, got {call_args[0][1]}"
                )

    def test_empty_skill_metrics_exits_zero(
        self, combined_with_models, gap_tuples,
    ):
    ...
    def test_empty_forecasts_for_gaps_exits_zero(
        self, combined_with_models, gap_tuples, skill_stats,
    ):
    ...
    def test_no_matching_forecast_rows_exits_zero(
        ...
        non_matching_forecasts = _make_forecasts([
            {'year': 2025, 'month': 6, 'code': '10001',
             'model_short': 'LR', 'q50': 200.0, 'q05': 180.0,
             'q95': 220.0, 'valid_from': '2025-06-01',
             'valid_to': '2025-06-30', 'date': '2025-06-01', 'flag': 0},
        ])
    ...
    def test_save_error_causes_exit_one(
        ...
        mock_file_writer.save_monthly_forecast_data.return_value = (
            "Error: disk full"
        )
    ...
    def test_multi_station_gaps(self, skill_stats):
        ...
        forecasts = _make_forecasts([
            ...
            {'year': 2025, 'month': 1, 'code': '10002',
             'model_short': 'LR', 'q50': 200.0, 'q05': 180.0,
             'q95': 220.0, 'valid_from': '2025-01-01',
             'valid_to': '2025-01-31', 'date': '2025-01-01', 'flag': 0},
        ])
        em_rows = pd.DataFrame([
            {'year': 2025, 'month': 1, 'code': '10001',
             'model_short': 'EM', 'forecasted_discharge': 100.0,
             'month_in_year': 1},
            {'year': 2025, 'month': 1, 'code': '10002',
             'model_short': 'EM', 'forecasted_discharge': 200.0,
             'month_in_year': 1},
        ])
        ensemble_output = pd.concat(
            [forecasts, em_rows], ignore_index=True,
        )
        ...
        with patch.dict(sys.modules, {}):
            ...
            assert len(em_saved) == 2, (
                f"Expected 2 EM rows (one per station), got {len(em_saved)}"
            )
