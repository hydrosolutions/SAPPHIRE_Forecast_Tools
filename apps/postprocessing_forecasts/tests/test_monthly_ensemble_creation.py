...
def _two_model_forecasts():
    ...
    return _make_forecasts([
        ('S1', 2025, 3, 3, 'LR',
         100.0, 80.0, 85.0, 90.0, 100.0, 110.0, 115.0, 120.0,
         '2025-03-01', '2025-03-31', '2025-03-01', 0),
        ('S1', 2025, 3, 3, 'TFT',
         120.0, 90.0, 95.0, 100.0, 120.0, 130.0, 135.0, 140.0,
         '2025-03-01', '2025-03-31', '2025-03-01', 0),
    ])
...
class TestEMCreation:
    ...
    def test_em_discharge_is_mean_of_qualified_models(self):
        ...
        assert em_rows.iloc[0]['forecasted_discharge'] == pytest.approx(
            expected
        )
    ...
    def test_em_excludes_baseline_models(self):
        ...
        em_new = result[
            (result['model_short'] == 'EM')
            & (result['forecasted_discharge'] != 999.0)
        ]
        assert len(em_new) == 1
    ...
    def test_em_three_models_qualified(self):
        ...
        forecasts = _make_forecasts([
            ...
            ('S1', 2025, 3, 3, 'TFT',
             ...
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
            ('S1', 2025, 3, 3, 'TiDE',
             130.0, 100.0, 105.0, 110.0, 130.0, 140.0, 145.0, 150.0,
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
        ])
...
class TestSkilledMeanCreation:
    ...
    def test_skilled_mean_is_mae_weighted(self):
        ...
        forecasts = _make_forecasts([
            ('S1', 2025, 3, 3, 'LR',
             ...
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
            ('S1', 2025, 3, 3, 'TFT',
             200.0, 160.0, 170.0, 180.0, 200.0, 210.0, 215.0, 220.0,
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
        ])
        ...
        assert sm_rows.iloc[0]['forecasted_discharge'] == pytest.approx(
            expected, rel=1e-4
        )
    ...
    def test_skilled_mean_excludes_baselines(self):
        ...
        sm_new = result[
            (result['model_short'] == 'Skilled Mean')
            & (result['forecasted_discharge'] != 999.0)
        ]
        assert len(sm_new) == 1
    ...
    def test_naive_mean_is_simple_average(self):
        ...
        forecasts = _make_forecasts([
            ...
            ('S1', 2025, 3, 3, 'TFT',
             ...
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
            ('S1', 2025, 3, 3, 'TiDE',
             150.0, 120.0, 125.0, 130.0, 150.0, 160.0, 165.0, 170.0,
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
        ])
...
class TestEdgeCases:
    ...
    def test_all_quantile_columns_propagated(self):
        ...
        for ensemble_type in ('EM', 'Skilled Mean', 'Naive Mean'):
            ...
            if rows.empty:
                continue
            for qc in qcols:
                assert qc in rows.columns, (
                    f"{qc} missing from {ensemble_type} rows"
                )
                val = rows.iloc[0][qc]
                assert pd.notna(val), (
                    f"{qc} is NaN in {ensemble_type} row"
                )
    ...
    def test_multi_station_independent_ensembles(self):
        ...
        forecasts = _make_forecasts([
            ...
            ('S1', 2025, 3, 3, 'TFT',
             ...
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
            ('S2', 2025, 3, 3, 'LR',
             200.0, 180.0, 185.0, 190.0, 200.0, 210.0, 215.0, 220.0,
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
            ('S2', 2025, 3, 3, 'TFT',
             240.0, 210.0, 215.0, 220.0, 240.0, 250.0, 255.0, 260.0,
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
        ])
        ...
        em_s1 = result[
            (result['model_short'] == 'EM') & (result['code'] == 'S1')
        ]
        em_s2 = result[
            (result['model_short'] == 'EM') & (result['code'] == 'S2')
        ]
        assert len(em_s1) == 1
        assert len(em_s2) == 1
    ...
    def test_multi_month_independent_ensembles(self):
        ...
        forecasts = _make_forecasts([
            ...
            ('S1', 2025, 4, 4, 'LR',
             200.0, 180.0, 185.0, 190.0, 200.0, 210.0, 215.0, 220.0,
             '2025-04-01', '2025-04-30', '2025-04-01', 0),
            ('S1', 2025, 4, 4, 'TFT',
             240.0, 210.0, 215.0, 220.0, 240.0, 250.0, 255.0, 260.0,
             '2025-04-01', '2025-04-30', '2025-04-01', 0),
        ])
        ...
        em_m3 = result[
            (result['model_short'] == 'EM') & (result['month'] == 3)
        ]
        em_m4 = result[
            (result['model_short'] == 'EM') & (result['month'] == 4)
        ]
        assert len(em_m3) == 1
        assert len(em_m4) == 1
    ...
    def test_nan_forecasted_discharge_dropped(self):
        ...
        forecasts = _make_forecasts([
            ('S1', 2025, 3, 3, 'LR',
             ...
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
            ('S1', 2025, 3, 3, 'TFT',
             float('nan'), 90.0, 95.0, 100.0, float('nan'), 130.0,
             135.0, 140.0,
             '2025-03-01', '2025-03-31', '2025-03-01', 0),
            ('S1', 2025, 3, 3, 'TiDE',
             130.0, 100.0, 105.0, 110.0, 130.0, 140.0, 145.0, 150.0,
