...
class TestTripleGroupbyVsSinglePass:
    ...
    def test_bench_triple_groupby(self):
        ...
        skill_stats = (
            self.df.groupby(self.group_keys)[["discharge_avg", "forecasted_discharge"]]
            .apply(
        ...
        accuracy_stats = (
            self.df.groupby(self.group_keys)[["discharge_avg", "forecasted_discharge", "delta"]]
            .apply(
        ...
        assert len(result) == 14400, (
            f"Expected 14400 groups (72 pentads x 50 stations x 4 models), got {len(result)}"
        )
    ...
    def test_bench_single_pass(self):
        ...
        if not hasattr(skill_metrics, "calculate_all_skill_metrics"):
            ...
            assert len(skill_stats) == 14400, f"Expected 14400 groups, got {len(skill_stats)}"
            return
        ...
        assert len(result) == 14400, f"Expected 14400 groups, got {len(result)}"
...
class TestIsinVsMerge:
    ...
    def setup_data(self):
        ...
        keys = self.skill_metrics_df[["pentad_in_year", "code", "model_short"]].drop_duplicates()
    ...
    def test_bench_isin_filter(self):
        ...
        assert len(result) >= 72000, f"isin should match at least 72000 rows, got {len(result)}"
    ...
    def test_bench_merge_filter(self):
        ...
        assert len(result) == 72000, f"Expected 72000 rows from inner merge, got {len(result)}"
...
class TestIterrowsVsVectorized:
    ...
    def test_bench_vectorized(self):
        ...
        df = self.df.copy()
        df["model_type"] = (
            df["model_short"].str.upper().map(model_type_map).fillna(df["model_short"])
        )
