...
# Ensure the real horizon_config is importable (needed for module-level constants)
import src.horizon_config as _real_horizon_config  # noqa: F401
...
def create_combined_forecast(
    lr_data, tft_data, tide_data, tsmixer_data, arima_data, sample_data, output_path
):
    ...
    for code in codes:
        for date in dates:
            ...
            # Get TSMixer data
            tsmixer_row = tsmixer_data[
                (tsmixer_data["code"] == code) & (tsmixer_data["date"] == date)
            ]
...
def test_combined_forecast_consistency(setup_test_paths, sample_data, tmp_path):
    ...
    expected_df = create_combined_forecast(
        lr_data, tft_data, tide_data, tsmixer_data, arima_data, sample_data, paths["output_file"]
    )
    ...
    # The unified functions (calculate_skill_metrics, save_forecast_data,
    # save_skill_metrics) are called once per horizon. In BOTH mode (default),
    # each is called twice: once for pentad, once for decad. We use
    # side_effect lists to return different values for each call.
    with (
        patch("postprocessing_forecasts.postprocessing_forecasts.sl.load_environment"),
        patch(
            "postprocessing_forecasts.postprocessing_forecasts.sl.read_observed_and_modelled_data_pentade"
        ) as mock_read,
        patch(
            "postprocessing_forecasts.postprocessing_forecasts.sl.read_observed_and_modelled_data_decade"
        ) as mock_read_decade,
        patch(
            "postprocessing_forecasts.postprocessing_forecasts.skill_metrics.calculate_skill_metrics"
        ) as mock_calc,
        patch(
            "postprocessing_forecasts.postprocessing_forecasts.file_writer.save_forecast_data"
        ) as mock_save,
        patch(
            "postprocessing_forecasts.postprocessing_forecasts.file_writer.save_skill_metrics"
        ) as mock_save_skill,
    ):
        # Configure mocks for pentad read
        mock_read.return_value = (
            observed_data,
            pd.concat([lr_data, tft_data, tide_data, tsmixer_data, arima_data]),
        )

        # Configure mocks for decade read (return empty data)
        mock_read_decade.return_value = (pd.DataFrame(), pd.DataFrame())

        # calculate_skill_metrics is called twice (pentad then decad)
        mock_calc.side_effect = [
            (pd.DataFrame(), expected_df, None),  # pentad call
            (pd.DataFrame(), pd.DataFrame(), None),  # decad call
        ]

        # save_forecast_data is called twice (pentad then decad)
        def side_effect_save(config, df):
            """Save only the pentad call to the output file."""
            if config.name == "pentad":
                df.to_csv(paths["output_file"], index=False)
            return None

        mock_save.side_effect = side_effect_save

        # save_skill_metrics is called twice (pentad then decad) — both return None
        mock_save_skill.return_value = None

        # Run the postprocessing function
        with contextlib.suppress(SystemExit):
            postprocessing_forecasts()

        # Verify unified mocks were called twice (once per horizon in BOTH mode)
        assert mock_calc.call_count == 2
        assert mock_save.call_count == 2
        assert mock_save_skill.call_count == 2
        ...
        for col in ["forecasted_discharge"]:
            if col in actual_df.columns:
                try:
                    ...
                    non_numeric = (
                        pd.to_numeric(actual_df[col], errors="coerce").isna()
                        & ~actual_df[col].isna()
                    )
                    if any(non_numeric):
        ...
        if "date" in actual_df.columns:
            ...
            print(
                f"Expected dates: {[pd.Timestamp(d).strftime('%Y-%m-%d') for d in sample_data['dates']]}"
            )
            print(
                f"Actual dates: {pd.to_datetime(actual_df['date']).dt.strftime('%Y-%m-%d').unique()}"
            )
        ...
        for code in sample_data["codes"]:
            for date_obj in sample_data["dates"]:
                ...
                code_str = str(code)
                date_code_data = actual_df[
                    (actual_df["code"].astype(str) == code_str)
        ...
        for code in sample_data["codes"]:
            for date_obj in sample_data["dates"]:
                ...
                # Convert code to string for comparison
                code_str = str(code)
                mask = (actual_df["code"].astype(str) == code_str) & (
                    pd.to_datetime(actual_df["date"]).dt.strftime("%Y-%m-%d") == date_str
                )
                ...
                if not em_rows.empty and len(model_forecasts) > 0:
                    ...
                    if abs(expected_em - actual_em) > tolerance:
                        pytest.fail(
                            f"Ensemble mean mismatch for {code} on {date_str}: expected {expected_em}, got {actual_em}"
                        )
                else:
                ...
                if not ne_rows.empty:
                    ...
                    ml_forecasts = {
                        k: v for k, v in model_forecasts.items() if k in ["TFT", "TiDE", "TSMixer"]
                    }
                    if ml_forecasts:
                        ...
                        if abs(expected_ne - actual_ne) > tolerance:
                            pytest.fail(
                                f"Neural ensemble mismatch for {code} on {date_str}: expected {expected_ne}, got {actual_ne}"
                            )
                    else:
                        print(
                            f"  No neural models for ensemble calculation for {code} on {date_str}"
                        )
                else:
