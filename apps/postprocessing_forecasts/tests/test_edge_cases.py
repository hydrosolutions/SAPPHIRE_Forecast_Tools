...
from test_constants import DEFAULT_THRESHOLDS


def _make_ensemble_pentad(forecasts, skill_stats):
...
class TestEmptyAndSingleRowData:
    ...
    def test_ensemble_single_model_rejected(self):
        ...
        with patch.dict(os.environ, DEFAULT_THRESHOLDS):
            joint, _ = _make_ensemble_pentad(forecasts, skill)
        assert "EM" not in joint["model_short"].values, "Single-model ensemble should be discarded"
...
class TestNaNHandling:
    ...
    def test_filter_nan_sdivsigma_excluded(self):
        ...
        assert len(result) == 0, "NaN sdivsigma should fail the < 0.6 comparison"
    ...
    def test_ensemble_all_nan_discharge(self):
        ...
        forecasts = pd.DataFrame(
            [
                _make_forecast_row("10001", "2024-01-05", 1, "1", "LR", np.nan),
                _make_forecast_row("10001", "2024-01-05", 1, "1", "TFT", np.nan),
            ]
        )
        pd.DataFrame(
            {
    ...
    def test_all_nan_discharge_preserves_original_rows(self):
        ...
        assert "EM" not in joint["model_short"].values
        assert len(joint) == 2, f"Original 2 rows must be preserved, got {len(joint)}"
    ...
    def test_multi_station_all_nan_one_valid_other(self):
        ...
        forecasts = pd.DataFrame(
            [
                ...
                _make_forecast_row("10002", "2024-01-05", 1, "1", "LR", 100.0),
                _make_forecast_row("10002", "2024-01-05", 1, "1", "TFT", 110.0),
            ]
        )
        pd.DataFrame(
            {
        ...
        assert len(em_rows) == 1, f"Expected 1 EM row (station 10002 only), got {len(em_rows)}"
...
class TestDischargeValueBoundaries:
    ...
    def _run_ensemble(self, lr_val, tft_val, skill, observed):
        forecasts = pd.DataFrame(
            [
                _make_forecast_row("10001", "2024-01-05", 1, "1", "LR", lr_val),
                _make_forecast_row("10001", "2024-01-05", 1, "1", "TFT", tft_val),
            ]
        )
        with patch.dict(os.environ, DEFAULT_THRESHOLDS):
            joint, _ = _make_ensemble_pentad(forecasts, skill)
    ...
    def test_very_large_discharge(self, _two_model_skill, _observed):
        ...
        em = self._run_ensemble(10000.0, 12000.0, _two_model_skill, _observed)
        assert len(em) == 1
        assert abs(em.iloc[0]["forecasted_discharge"] - 11000.0) < 0.01
    ...
    def test_identical_discharge_all_models(self):
        ...
        forecasts = pd.DataFrame(
            [
                _make_forecast_row("10001", "2024-01-05", 1, "1", "LR", 105.0),
                _make_forecast_row("10001", "2024-01-05", 1, "1", "TFT", 105.0),
                _make_forecast_row("10001", "2024-01-05", 1, "1", "TiDE", 105.0),
            ]
        )
        pd.DataFrame(
            {
...
class TestDuplicateHandling:
    ...
    def test_duplicate_forecast_rows(self):
        ...
        forecasts = pd.DataFrame(
            [
                _make_forecast_row("10001", "2024-01-05", 1, "1", "LR", 100.0),
                _make_forecast_row("10001", "2024-01-05", 1, "1", "LR", 120.0),
    ...
    def test_duplicate_skill_stats_rows(self):
        ...
        assert abs(em.iloc[0]["forecasted_discharge"] - 105.0) < 0.01, "EM = mean(100, 110) = 105"
...
class TestThresholdBehavior:
    ...
    def test_all_thresholds_must_pass(self):
        ...
        assert len(result) == 0, "Failing any single threshold should exclude the model"
    ...
    def test_custom_relaxed_thresholds_include_all(self):
        ...
        df = pd.DataFrame(
            [
                _make_skill_row("10001", 1, "LR", sdivsigma=0.3, nse=0.95, accuracy=0.95),
                _make_skill_row("10001", 1, "TFT", sdivsigma=0.4, nse=0.9, accuracy=0.88),
                _make_skill_row("10001", 1, "TiDE", sdivsigma=0.9, nse=0.5, accuracy=0.6),
            ]
        )
        result = filter_for_highly_skilled_forecasts(
            df,
            threshold_sdivsigma=1.0,
            threshold_accuracy=0.5,
            threshold_nse=0.4,
        )
        assert len(result) == 3, "Relaxed thresholds should include all 3 models"
    ...
    def test_custom_strict_sdivsigma_only(self):
        ...
        df = pd.DataFrame(
            [
                _make_skill_row("10001", 1, "LR", sdivsigma=0.15, nse=0.85, accuracy=0.82),
                _make_skill_row("10001", 1, "TFT", sdivsigma=0.4, nse=0.9, accuracy=0.88),
            ]
        )
        result = filter_for_highly_skilled_forecasts(
            df,
            threshold_sdivsigma=0.2,
            threshold_accuracy=0.0,
            threshold_nse=0.0,
        )
        assert len(result) == 1
    ...
    def test_custom_thresholds_via_env_vars(self):
        ...
        assert len(result) == 3, "Relaxed env-var thresholds should include all models"
    ...
    def test_mixed_threshold_disabling(self):
        ...
        df = pd.DataFrame(
            [
                _make_skill_row(
                    "10001", 1, "LR", sdivsigma=0.9, nse=0.95, accuracy=0.95
                ),  # bad sdivsigma, good others
                _make_skill_row(
                    "10001", 1, "TFT", sdivsigma=0.8, nse=0.9, accuracy=0.88
                ),  # bad sdivsigma, good others
                _make_skill_row(
                    "10001", 1, "TiDE", sdivsigma=0.1, nse=0.5, accuracy=0.6
                ),  # good sdivsigma, bad others
            ]
        )
        result = filter_for_highly_skilled_forecasts(
            df,
        ...
        assert len(result) == 2, "sdivsigma disabled: LR+TFT pass on accuracy+nse, TiDE fails"
...
class TestPeriodColCoercion:
    ...
    def test_string_period_values_coerced(self):
        ...
        forecasts["pentad_in_year"] = forecasts["pentad_in_year"].astype(str)
...
class TestDeltaEdgeCases:
    ...
    def test_zero_delta_strict_matching(self):
        ...
        assert result["accuracy"] == 0.5, "delta=0: only exact match (row 0) counts, accuracy = 0.5"
    ...
    def test_negative_delta_produces_nan(self):
        ...
        result = skill_metrics.calculate_all_skill_metrics(data, "obs", "sim", "delta")
        assert result["n_pairs"] == 2
        assert np.isnan(result["delta"]), "Negative delta fails 0 <= delta validation -> NaN"
        assert np.isnan(result["accuracy"]), "When delta is invalid, accuracy is also set to NaN"
...
class TestNaTDatesInGapDetector:
    ...
    def test_nat_dates_dropped_gracefully(self):
        ...
        df = pd.DataFrame(
            {
                "date": pd.to_datetime(["2024-01-05", pd.NaT, "2024-01-10"]),
                "code": ["10001", "10001", "10001"],
