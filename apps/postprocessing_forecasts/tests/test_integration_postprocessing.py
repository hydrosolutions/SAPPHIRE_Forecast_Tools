...
class TestOperationalDataRouting:
    ...
    def test_ensemble_created_and_written_to_csv(
        ...
        file_writer.save_forecast_data(PENTAD, joint)
...
class TestDecadalOperationalPipeline:
    ...
    def test_decadal_ensemble_created_and_written_to_csv(
        ...
        # Save to CSV
        file_writer.save_forecast_data(DECAD, joint)
...
# ---------------------------------------------------------------------------
# TestMultiStationMultiGapMaintenance (Gap #3)
# ---------------------------------------------------------------------------
class TestMultiStationMultiGapMaintenance:
    """Multi-station gap-fill: station A gap on date X, station B gap on Y.

    Verifies that the date+code filtering in _fill_gaps_for_horizon
    (modelled['date'].isin(gap_dates) & modelled['code'].isin(gap_codes))
    correctly selects the right rows for each station's gap.
    """

    def test_two_stations_different_gap_dates(self, env_setup):
        """Station A missing EM on Jan 5, Station B missing on Jan 10.

        Combined CSV:
        - 15001: LR+TFT+EM on Jan 10, LR+TFT (no EM) on Jan 5
        - 15002: LR+TFT+EM on Jan 5,  LR+TFT (no EM) on Jan 10
        Gap detection should find: (Jan 5, 15001) and (Jan 10, 15002).
        Ensemble fill should create EM for each.
        """
        tmp_path = env_setup
        rows = []
...
# ---------------------------------------------------------------------------
# TestNeuralEnsembleExclusion (Gap #5)
# ---------------------------------------------------------------------------
class TestNeuralEnsembleExclusion:
    """NE (Neural Ensemble) must be excluded from EM candidate pool.

    ensemble_calculator.py line 202 filters out model_short == 'NE'.
    This integration test verifies the full pipeline handles NE correctly:
    NE passes skill thresholds but is NOT included in EM computation.
    """
...
class TestRecalculateWithRealisticData:
    """Test #8: Feed realistic data through calculate_skill_metrics(PENTAD).
    ...
    def test_skill_stats_shape_and_groups(self, recalc_env, realistic_data):
        ...
        skill_stats, joint, _ = skill_metrics.calculate_skill_metrics(PENTAD, observed, simulated)
    ...
    def test_save_writes_csv_and_api(self, recalc_env, realistic_data):
        ...
        with patch.object(api_writer, "SAPPHIRE_API_AVAILABLE", False):
            file_writer.save_skill_metrics(PENTAD, skill_stats)
        ...
        # Need pentad_in_month for save_forecast_data(PENTAD, ...)
        ...
        with patch.object(api_writer, "SAPPHIRE_API_AVAILABLE", False):
            file_writer.save_forecast_data(PENTAD, joint_with_pim)
...
class TestSkillMetricSavePath:
    """Test #9: Verify save_skill_metrics(PENTAD) writes CSV correctly
    ...
    def test_csv_columns_and_sort_order(
        ...
        with patch.object(api_writer, "SAPPHIRE_API_AVAILABLE", False):
            file_writer.save_skill_metrics(PENTAD, known_skill_stats)
    ...
    def test_code_cleaned_no_dot_zero(self, save_env):
        ...
        with patch.object(api_writer, "SAPPHIRE_API_AVAILABLE", False):
            file_writer.save_skill_metrics(PENTAD, data)
    ...
    def test_api_write_called_with_correct_args(
        ...
        with (
            ...
            file_writer.save_skill_metrics(PENTAD, known_skill_stats, year=2024)
...
class TestRecalculateSkillMetricsIntegration:
    ...
    def test_pentad_mode_calls_correct_functions(self, env_setup):
        ...
        with (
            ...
            patch(
                "src.skill_metrics.calculate_skill_metrics",
                return_value=(
                    pd.DataFrame(),
                    pd.DataFrame(),
                    None,
            ...
            patch(
                "src.file_writer.save_forecast_data",
                return_value=None,
            ) as mock_save_fc,
            patch(
                "src.file_writer.save_skill_metrics",
                return_value=None,
            ) as mock_save_sk,
            patch.dict(
                os.environ,
...
# ---------------------------------------------------------------------------
# TestEnsembleSkillMetricVerification (#10)
# ---------------------------------------------------------------------------
class TestEnsembleSkillMetricVerification:
    """Verify exact numerical values of ensemble skill metrics.

    Uses 3 dates in pentad 1 with known LR, TFT, observed, and delta
    values so the EM (= mean of LR, TFT) and its skill metrics can be
    hand-calculated and verified exactly.

    Hand calculations (see inline comments for derivation):
        EM forecasts : [105, 110, 120]   (mean of LR, TFT)
        Observed     : [100, 106, 118]
        Delta        : [5, 5, 5]

        differences  = obs - sim = [-5, -4, -2]
        abs_diff     = [5, 4, 2]
        MAE          = (5 + 4 + 2) / 3 = 11/3 ≈ 3.6667
        accuracy     = all abs_diff <= 5  =>  1.0
        delta        = 5.0
        n_pairs      = 3

        obs_mean     = (100 + 106 + 118) / 3 = 108.0
        SS_obs       = (100-108)^2 + (106-108)^2 + (118-108)^2
                     = 64 + 4 + 100 = 168
        SS_res       = 25 + 16 + 4 = 45
        NSE          = 1 - 45/168 = 123/168 ≈ 0.732143

        sigma_obs    = sqrt(168 / 2) = sqrt(84) ≈ 9.16515
        RMSE_ddof1   = sqrt(45 / 2) = sqrt(22.5) ≈ 4.74342
        sdivsigma    = sqrt(22.5) / sqrt(84) = sqrt(22.5/84) ≈ 0.51755
    """
    ...
    def test_ensemble_skill_metrics_exact_values(self, metric_env):
        """EM row skill metrics match hand-calculated values.
...
# ---------------------------------------------------------------------------
# TestMultiEntityEnsembleSkillStats (Gap #4)
# ---------------------------------------------------------------------------
class TestMultiEntityEnsembleSkillStats:
    """Verify skill_stats_with_ensemble has correct EM rows for multi-entity.

    When 2 stations x 2 pentads are processed, the second return value
    (skill_stats_with_ensemble) should contain EM skill rows only for
    stations/pentads that actually qualify for ensemble.
    """

    def test_multi_station_multi_pentad_em_skill_rows(self, env_setup):
        """Station A qualifies for EM in pentad 1+2; station B does not.
        ...
        # Use varying values so observed has nonzero variance per pentad
...
# ---------------------------------------------------------------------------
# TestLeapYearBoundary (#11)
# ---------------------------------------------------------------------------
class TestLeapYearBoundary:
    """Leap year date handling: Feb 29 and Mar 1 get correct pentad values.

    Feb 29 in a leap year:
        pentad_in_month = min((29-1)//5 + 1, 6) = min(6, 6) = 6
        pentad_in_year  = (2-1)*6 + 6 = 12

    Mar 1:
        pentad_in_month = min((1-1)//5 + 1, 6) = min(1, 6) = 1
        pentad_in_year  = (3-1)*6 + 1 = 13
    """
...
# ---------------------------------------------------------------------------
# TestDecadalMaintenanceFullGapFill (#20)
# ---------------------------------------------------------------------------
class TestDecadalMaintenanceFullGapFill:
    """End-to-end decadal gap-fill: detect → ensemble → verify.

    Mirrors TestMaintenanceFullGapFill but uses decad columns and functions.
    """
    ...
    def test_decadal_gap_detected_ensemble_created_and_saved(
        ...
        Combined CSV has 2 dates: Jan 10 has EM, Jan 20 is missing EM.
        After gap-fill, Jan 20 should have an EM row.
        """
        tmp_path = decad_gap_env
...
# ---------------------------------------------------------------------------
# TestDecadalRecalculateWithRealisticData (#21)
# ---------------------------------------------------------------------------
class TestDecadalRecalculateWithRealisticData:
    """Test #21: Feed realistic data through calculate_skill_metrics(DECAD).

    Mirrors TestRecalculateWithRealisticData but for decadal resolution.
    3 stations x 2 decads x 2 models, with known observed values.
    """
    ...
    @pytest.fixture
    def decad_realistic_data(self):
        """3 stations x 2 decads x 2 models, 5 dates per decad.
    ...
    def test_decad_skill_stats_shape(
        ...
        n_pairs should be 5 for each group (5 dates per decad).
        """
    ...
    def test_decad_save_writes_csv(
        ...
        # Save skill metrics
        with patch.object(api_writer, "SAPPHIRE_API_AVAILABLE", False):
            file_writer.save_skill_metrics(DECAD, skill_stats)
        ...
        with patch.object(api_writer, "SAPPHIRE_API_AVAILABLE", False):
            file_writer.save_forecast_data(DECAD, joint_with_dim)
