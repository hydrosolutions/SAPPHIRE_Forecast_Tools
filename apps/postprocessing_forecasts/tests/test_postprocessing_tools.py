...
def test_log_most_recent_forecasts_pentad(
    tmp_path, monkeypatch, sample_data, expected_pivot_data
):
...
def test_log_most_recent_forecasts_pentad_empty_data(
...
def test_log_most_recent_forecasts_pentad_multiple_dates(
    tmp_path, monkeypatch, sample_data
):
...
def test_log_most_recent_forecasts_pentad_missing_code_no_matching_date(
...
def test_log_most_recent_forecasts_decade(
    tmp_path, monkeypatch, sample_decade_data
):
    ...
    # Verify CSV was written
    forecast_dir = tmp_path / 'forecast_logs'
    csv_files = list(forecast_dir.glob('recent_model_forecasts_decade_*.csv'))
    assert len(csv_files) == 1


def test_log_most_recent_forecasts_decade_empty_data(
    tmp_path, monkeypatch, sample_decade_data
):
...
class TestLogMostRecentForecastsMonthly:
    ...
    def test_happy_path(self, tmp_path, monkeypatch,
        ...
        # Verify CSV was written
        forecast_dir = tmp_path / 'forecast_logs'
        csv_files = list(forecast_dir.glob(
            'recent_model_forecasts_monthly_*.csv'
        ))
        assert len(csv_files) == 1
    ...
    def test_uses_most_recent_year_month(
        self, tmp_path, monkeypatch, sample_monthly_data,
    ):
