...
def _make_pentad_ensemble(forecasts, skill_stats, observed):
...
class TestEnsembleCalculatorCharacterization:
    ...
    def test_create_ensemble_works_without_model_long_input(
        ...
        joint, skill_out = _make_pentad_ensemble(
    ...
    def test_create_ensemble_preserves_discharge_values(
        ...
        assert em_rows.iloc[1]['forecasted_discharge'] == pytest.approx(
            125.0, abs=1e-5
        )
    ...
    def test_single_model_ensemble_discarded(self, observed_two_dates):
        ...
        joint, _ = _make_pentad_ensemble(
            forecasts, skill_stats, observed_two_dates
        )
...
class TestSkillMetricsCharacterization:
    ...
    def test_pentad_groupby_uses_model_short(
        self, observed_sm, simulated_sm
    ):
        """skill_stats groups by model_short, not model_long."""
        skill_stats, _, _ = skill_metrics.calculate_skill_metrics_pentad(
            observed_sm, simulated_sm
        )
    ...
    def test_pentad_individual_model_metrics_correct(
        self, observed_sm, simulated_sm
    ):
    ...
    def test_pentad_ensemble_discharge_mean_correct(
        self, observed_sm, simulated_sm, monkeypatch
    ):
        ...
        em_rows = joint[
            (joint['model_short'] == 'EM') &
            (joint['code'] == '123')
        ].sort_values('date')
        assert len(em_rows) >= 2
        # 2022-01-01: mean(10.2, 10.1) = 10.15
        em_jan1 = em_rows[
            em_rows['date'] == pd.Timestamp('2022-01-01')
        ]
        assert em_jan1.iloc[0]['forecasted_discharge'] == pytest.approx(
            10.15, abs=1e-5
        )
...
class TestCompositionAgg:
    ...
    def test_four_models(self):
        result = composition_agg(
            pd.Series(['TSMixer', 'TiDE', 'LR', 'TFT'])
        )
