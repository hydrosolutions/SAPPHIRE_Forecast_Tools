...
class TimingStats:
    ...
    def summary(self):
        ...
        for section, times in self.timings.items():
            ...
            results.append(
                {"section": section, "total_time": total, "avg_time": avg, "calls": len(times)}
            )
...
def log_most_recent_forecasts(config, modelled_data):
    ...
    Args:
        ...
        modelled_data: DataFrame containing the modelled forecast data.
    ...
    if missing_models:
        logger.info(
            f"Models from historical data not present in recent forecasts: {', '.join(missing_models)}"
        )
    ...
    if missing_codes:
        logger.info(
            f"Found {len(missing_codes)} station codes from historical data not present in recent forecasts"
        )

        # Create rows for missing codes
        missing_rows = []
        for code in missing_codes:
            # Find the most recent period for this code in the full dataset
            ...
            if not code_data.empty:
                ...
                if filtered_by_date.empty:
                    ...
                    continue
                period_val = filtered_by_date[period_in_month_col].iloc[0]

                # Create a row with just the code, date, and period
                new_row = {
                    "code": code,
                    "date": most_recent_date,
                    period_in_month_col: period_val,
                }
    ...
    forecast_dir = os.path.join(os.getenv("ieasyforecast_intermediate_data_path"), "forecast_logs")
    os.makedirs(forecast_dir, exist_ok=True)

    # Save pivoted data to CSV
    forecast_file = os.path.join(
        forecast_dir, f"recent_model_forecasts_{horizon}_{most_recent_date.strftime('%Y%m%d')}.csv"
    )
...
def log_most_recent_forecasts_monthly(modelled_data):
    ...
    latest_year_data = modelled_data[modelled_data["year"] == max_year]
    ...
    recent = modelled_data[
        (modelled_data["year"] == max_year) & (modelled_data["month_in_year"] == max_month)
    ]

    if recent.empty:
        logger.warning("No forecasts for year=%d month=%d", max_year, max_month)
        return pd.DataFrame()
    ...
    # Save pivoted data to CSV
    forecast_file = os.path.join(
        forecast_dir, f"recent_model_forecasts_monthly_{max_year}{max_month:02d}.csv"
    )
    pivoted.to_csv(forecast_file, index=False)

    logger.info("Recent monthly forecasts saved to: %s", forecast_file)
