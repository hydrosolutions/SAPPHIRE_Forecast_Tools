...
logger = logging.getLogger("initialize_ml_tool")
...
from scr.utils_ml_forecast import SAPPHIRE_API_AVAILABLE, _write_ml_forecast_to_api
...
def main():
    ...
    # --------------------------------------------------------------------
    print(
        f"Initialize Machine Learning Model {MODEL_TO_USE}: Produce Hindcast for the following time period"
    )
    ...
    pentad_hindcast_daily = call_hindcast_script(
        start_date, end_date, MODEL_TO_USE, intermediate_data_path, "PENTAD"
    )
    ...
    decad_hindcast_daily = call_hindcast_script(
        start_date, end_date, MODEL_TO_USE, intermediate_data_path, "DECAD"
    )
    ...
    print(
        f"The forecast files for model {MODEL_TO_USE} are saved in the directory: {OUTPUT_PATH_DISCHARGE}"
    )
