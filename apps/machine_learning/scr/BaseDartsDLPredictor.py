...
class BaseDartsDLPredictor(BasePredictor):
    ...
    def __init__(
        ...
        self.quantiles = self.dl_config.get(
            "quantiles",
            [
                0.05,
                0.1,
        ...
        self.exogene_covariates_cols = self.dl_config.get(
            "exogene_covariates_cols", ["P", "T", "PET", "daylight_hours"]
        )
        self.past_covariates_cols = self.dl_config.get(
            "past_covariates_cols", ["moving_avr_dis_3", "moving_avr_dis_5", "moving_avr_dis_10"]
        )
        self.future_covariates_cols = self.dl_config.get(
            "future_covariates_cols", ["P", "T", "PET", "daylight_hours"]
        )
        self.window_sizes = self.dl_config.get("window_sizes", [3, 5, 10])
        self.trainer_config = self.dl_config.get(
            "trainer_config", {"accelerator": "cpu", "logger": False}
        )
    ...
    def _scale_static_features(self):
        ...
        for col in self.static_features.columns.values:
            ...
            self.static_features[col] = self.scale_covariates(
                self.static_features, self.scaler_static, type=self.scaling_type_static, col=col
            )
    ...
    def scale_discharge(
        self, data: pd.DataFrame, scaler: pd.DataFrame | pd.Series, type: str = "standard"
    ) -> pd.Series:
        """
    ...
    def scale_covariates(
        self, data: pd.DataFrame, scaler: pd.DataFrame | pd.Series, type: str, col: str
    ) -> pd.Series:
        """
        ...
        if type == "standard":
            ...
            data_scaled = (data[col] - scaler.loc[col]["min"]) / (
                scaler.loc[col]["max"] - scaler.loc[col]["min"]
            )
        else:
            raise ValueError(f"Unknown scaling type: {type}")
    ...
    def calc_rolling_mean(self, df_rivers: pd.DataFrame, window: int) -> np.array:
        """
    ...
    def add_month(self, ts: TimeSeries) -> TimeSeries:
        """
    ...
    def create_prediction_df(self, predictions: TimeSeries, code: int) -> pd.DataFrame:
        """
    ...
    def _prepare_data(
        self, df_rivers_org: pd.DataFrame, df_covariates: pd.DataFrame, code: int
    ) -> tuple:
        """
    ...
    def predict(
        self, df_rivers_org: pd.DataFrame, df_covariates: pd.DataFrame, code: int, n: int
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            df_rivers, df_covariates, df_covariates_past = self._prepare_data(
                df_rivers_org=df_rivers_org, df_covariates=df_covariates, code=code
            )
