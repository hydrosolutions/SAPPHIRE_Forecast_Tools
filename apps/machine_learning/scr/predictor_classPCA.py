...
class PREDICTOR():
    ...
    def predict(self, df_rivers_org: pd.DataFrame, df_era5: pd.DataFrame, df_swe:pd.DataFrame, code: int, n:int, make_plot: bool = False):
        ...
        #just take the last input_chunk_length + 3 values
        df_rivers = df_rivers.iloc[-self.get_input_chunk_length()-15:]
        
        
        #scale the data
        df_rivers['discharge'] = self.scale_discharge(df_rivers, self.scaler_discharge.loc[code], type='standard')
        
        if not df_rivers['discharge'].isna().any():
            ssa = SingularSpectrumAnalysis(window_size=8)
            X_ssa = ssa.fit_transform(df_rivers['discharge'].values.reshape(1, -1))
            df_rivers['discharge'] = X_ssa[0][0] + X_ssa[0][1] 
        ...
        df_covariates_past['moving_average_discharge'] = self.calc_rolling_mean(df_rivers, df_era5, window=10)
        ...
        #past covariates 
        if df_swe is not None:
            covariates_past = TimeSeries.from_dataframe(df_covariates_past, time_col='date', value_cols = [ 'SWE','moving_average_discharge'], freq='1D')
        else:
            covariates_past = TimeSeries.from_dataframe(df_covariates_past, time_col='date', value_cols = ['moving_average_discharge'], freq='1D')
