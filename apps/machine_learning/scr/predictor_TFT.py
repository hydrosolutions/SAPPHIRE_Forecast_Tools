...
class PREDICTOR():
    ...
    def predict(self, df_rivers_org: pd.DataFrame, df_era5: pd.DataFrame, df_swe:pd.DataFrame, code: int, n:int, make_plot: bool = False):
        ...
        #past covariates 
        if df_swe is not None:
            covariates_past = TimeSeries.from_dataframe(df_covariates_past, time_col='date', value_cols = [ 'SWE',], freq='1D')
        else:
            covariates_past = TimeSeries.from_dataframe(df_covariates_past, time_col='date', 
                                                        value_cols = ['moving_avr_dis_3','moving_avr_dis_5','moving_avr_dis_10'], freq='1D')
