...
- [ ] **Remove retired entries that fail silently** — these were valid in earlier
  ...
  |---|---|---|
  | `run_periodic_maintenance.sh monthly_norms` | **Before INFRA-023 (merged 2026-09-07): failed silently, and the cron log looked successful.** The wrapper's argument guard rejected only an *empty* task type, so `monthly_norms` passed through to Luigi, where `RunPeriodicMaintenanceWorkflow.requires()` raised `ValueError: Unknown task_type`; the wrapper never exited on the compose status except on the `lt_recovery` branch, so it printed "task submitted" and **exited 0**. Every 1 January the long-horizon hydrograph view was silently not rebuilt. **Since INFRA-023 the wrapper rejects the name up front** with a non-zero exit naming the replacement — so once this deployment is updated the line starts failing *visibly* (that is the intended signal, not a new fault). Remove it either way. This is **INFRA-023** (`doc/plans/issues/archive/mid_prio_gi_draft_infra_yearly_monthly_norms_cron_unmapped.md`, Complete). | entry **(9)** `bin/yearly_runoff_hydrograph_aggregation.sh` |
