...
- [ ] **Remove retired entries that fail silently** — these were valid in earlier
  ...
  |---|---|---|
  | `run_periodic_maintenance.sh monthly_norms` | **Fails silently, and the cron log looks successful.** The wrapper's argument guard only rejects an *empty* task type (`bin/run_periodic_maintenance.sh:46-54`), so `monthly_norms` passes through to Luigi, where `RunPeriodicMaintenanceWorkflow.requires()` raises `ValueError: Unknown task_type` (`apps/pipeline/pipeline_docker.py:2077-2086`). The wrapper has no `set -e`; it *does* capture the compose status into `COMPOSE_STATUS` (`:169`) but `exit`s it only on the `lt_recovery` branch, so for the other task types it falls through the untaken `if` (`:177-193`) and **exits 0**, printing "task submitted". Every 1 January the long-horizon hydrograph view is silently not rebuilt. This is **INFRA-023** (`doc/plans/issues/mid_prio_gi_draft_infra_yearly_monthly_norms_cron_unmapped.md`). | entry **(9)** `bin/yearly_runoff_hydrograph_aggregation.sh` |
