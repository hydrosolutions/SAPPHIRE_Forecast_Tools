...
- [ ] Add the schedule below. Times are UTC; use the timezone reference table in `doc/deployment.md` > `Set up cron job` to pick an operational window that fits your hydromet (pentadal forecast usually runs in the morning local time, daily maintenance in the evening local time).
  ...
  # Yearly snow norm recalculation (31 August)
  ...
  # Yearly runoff hydrograph aggregation (January 1)
  # Replaces the retired `monthly_norms` Luigi task, which was removed from the
  # task map: run_periodic_maintenance.sh still accepts the retired name and
  # exits 0 without running anything (INFRA-023), so do not reinstate it.
