...
| **PP-040** | DECADE skill `n_pairs` starved on the Tajik server — **sparse ML forecast archive, NOT a code defect (proven)** | **High** | Draft (ops-only: finish tjhm archive) | [`high_prio_gi_draft_pp_decadal_skill_metrics_starved.md`](issues/high_prio_gi_draft_pp_decadal_skill_metrics_starved.md) | Observed mid-backfill on tjhm 2026-06-17. **PROVEN END-TO-END 2026-07-15:** 15/16 Tajik stations had ONE ML forecast row → n_pairs=1. Migrated the Tajik combined CSVs locally (124k rows) → stations went 1→570 rows; scoped recalc lifted a starved station's n_pairs from 0-1 to **14-15** across all ML models, both horizons, **zero code change.** A proposed issue-date/target-date pairing root cause was **REFUTED** — `discharge_avg` is forward-looking and NaN'd off issue dates (`forecast_library.py:790-855`), so both merge sides are issue-date indexed and `skill_metrics.py:2030` is correct. **Do NOT change the pairing.** Real cause: Tajik ML forecast archive is ~12x thinner per station than kyg (interrupted backfill). Fix = finish the ML archive migration (from-file combined CSVs), then recalc. Measure tjhm only to size the remaining work. |
...
*Last updated: 2026-07-14 — Added PREPQ-011 (runoff read-merge-write clobbers non-null discharge/predictor past row 100 — silent data loss, blocks the runoff backfill), PP-040 (DECADE skill metrics starved, n_pairs maxes at 1 despite a full archive), PP-041 (long_forecasts stale-aggregate invalidation, deferred P1b of PR #411), PP-042 (latent ensemble-exclusion display-form/DB-form mismatch, from PR #405 validation) and FD-018 (month_0 forecasts annotated with lead-1 skill). Earlier: 2026-06-19 — Added ML-016 (standalone run_locally.sh machine_learning crashes on empty SAPPHIRE_PREDICTION_MODE) + ML-017 (single missing ERA5 day → NaN cascade across all short-term ML; prepg interior-gap + ML covariate guard; found in local review tjhm+kghm). Earlier: 2026-06-16 — Added PP-037 (maintenance `model_short` KeyError on empty DECAD read, Ready — plan through two review cycles; Phase 1 is the minimum-safe production fix). 2026-04-16 — SEC-006 Draft→Review (PR #330); P-005/P-006 Complete; LTF-005 Draft→Review; added P-004, FD-014, PP-028b.*
