...
| **PP-040** | `fdc_flv` low-flow bias sign depends on unit/magnitude of observed discharge (denominator `sum(log(obs_low))` flips sign below 1.0), not just model bias — persisted, cross-station-comparable metric | **Medium** | Draft | [`mid_prio_gi_draft_pp_fdc_flv_unit_dependent_sign.md`](issues/mid_prio_gi_draft_pp_fdc_flv_unit_dependent_sign.md) | Follow-up to docstring+test fix (commit `6ca1537a`); id must be reconciled at merge (branch `fix_pp_informational_metrics`); owner decision needed (rename vs anchored Yilmaz Eq.4 + migration) |
