...
| **PP-040** | Skill pairing joins on issue date, not target date — starves `n_pairs` (DECADE worst-hit, maxes at 1) | **High** | Draft | [`high_prio_gi_draft_pp_decadal_skill_metrics_starved.md`](issues/high_prio_gi_draft_pp_decadal_skill_metrics_starved.md) | Observed in the Tajik (tjhm) backfill 2026-06-17. **Root cause found 2026-07-14** (out-of-loop review): `skill_metrics.py:2030` merges forecasts to observations on raw `[code, date]`, but forecast `date` = ISSUE date while the observation sits on the TARGET period — pairs survive only where they coincide. **NOT decade-specific** (pentad/decade share the merge); decade starves hardest given archive shape. Existing tests can't catch it (they observe on issue dates). Escalate if the fix changes the meaning of `Forecast.date`/`target` (service-owned). |
