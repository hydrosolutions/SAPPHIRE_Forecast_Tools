...
| **MIG-008** | From-file importer collapses quarter/season to `horizon_value=0` — can't reconstruct operational QUARTER hv1-4 / SEASON hv1-3 buckets | migration-toolkit | Mid | Draft | [`mid_prio_gi_draft_migration_long_forecast_quarter_season_horizon_value.md`](issues/mid_prio_gi_draft_migration_long_forecast_quarter_season_horizon_value.md) | Surfaced 2026-06-21 during the MIG-007 local season/quarter write. Single config per horizon (`quarter.json`/`seasonal_april.json`, lead 0) -> hv derivation yields only hv0. Quarter write held; season hv0 applied as clean additive gap-fill. Resolve after confirming how operational LT assigns quarter/season hv. |
