...
| **PP-044** | `Bulletin` schema has no per-site target period — reload heuristic in the dashboard is a best-effort guess, not a fix | **Medium** | Draft | [`mid_prio_gi_draft_pp_bulletin_target_period_field.md`](issues/mid_prio_gi_draft_pp_bulletin_target_period_field.md) | Service owner coordination (`sapphire/services/postprocessing`) — needs a discussion + Alembic migration before any code, per `CLAUDE.md` ownership boundary. Found during FD-018 review. |
