...
| **PP-045** | Missed boundary-day operational run strands short-term per-model period rows (PENTAD/DECADE); maintenance cannot heal (universe bounded by `combined`, write set never emits fresh per-model rows). Self-heals at next same-horizon boundary run **within-year** (if DAY archive + ML available), **permanent across year boundary**. | **High** | Draft — **awaiting A/B/C decision** | [`high_prio_gi_draft_pp_missed_boundary_period_gap.md`](issues/high_prio_gi_draft_pp_missed_boundary_period_gap.md) | Found + codex-verified 2026-07-17 (Tajik local, code `19999`). Options: A ops/docs, B backfill entrypoint, C self-healing operational. Deferred acceptance: kyg end-to-end verify (server down). Secondary decade-EM anomaly + yearless-key `get_latest_forecasts` collapse flagged as separate tickets. |
