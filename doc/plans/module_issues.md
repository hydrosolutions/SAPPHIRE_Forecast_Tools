...
| **PP-045** | Missed boundary-day operational run strands short-term per-model period rows (PENTAD/DECADE); maintenance cannot heal (universe bounded by `combined`, write set never emits fresh per-model rows). Self-heals at next same-horizon boundary run **within-year** (if DAY archive + ML available), **permanent across year boundary**. | **High** | Review — Option B implemented (branch `fix_postprocessing_boundary_gap`, not pushed) | [`review_gi_draft_pp_missed_boundary_period_gap.md`](issues/review_gi_draft_pp_missed_boundary_period_gap.md) | Found+codex-verified 2026-07-17; Option B (backfill CLI `backfill_period_forecasts.py`) shipped + reviewed 2026-07-23. Suite 1749 pass/0 fail; live Tajik dry-run heals 39 pentads/19 decads ×6 models. Deferred: kyg end-to-end (server down). Owner flag: API-only-by-default (`--write-csv` opt-in). Separate tickets to file: decade-EM skill-empty anomaly + yearless-key `get_latest_forecasts` collapse. |
