...
| **PP-040** | DECADE skill `n_pairs` starved on the Tajik server — **sparse ML forecast archive, NOT a code defect** | **High** | Draft (verify on server) | [`high_prio_gi_draft_pp_decadal_skill_metrics_starved.md`](issues/high_prio_gi_draft_pp_decadal_skill_metrics_starved.md) | Observed mid-backfill on tjhm 2026-06-17. **ROOT CAUSE PROVEN 2026-07-14: 15 of 16 Tajik stations have exactly ONE ML forecast row (both horizons) → n_pairs=1; the one station with a real archive reaches n_pairs=17.** Kyrgyz decade skill is healthy, so the decade path pairs fine. A proposed issue-date/target-date pairing root cause was **REFUTED** — `discharge_avg` is forward-looking and NaN'd off issue dates (`forecast_library.py:790-855`), so both merge sides are issue-date indexed and `skill_metrics.py:2030` is correct. **Do NOT change the pairing.** Real cause: Tajik ML forecast archive is ~12x thinner per station than kyg (interrupted backfill). Fix = finish the ML archive migration (from-file combined CSVs), then recalc. Measure tjhm only to size the remaining work. |
