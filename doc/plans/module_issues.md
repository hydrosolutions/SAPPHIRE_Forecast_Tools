...
| **PP-040** | DECADE skill metrics starved (`n_pairs` maxes at 1) despite a full decade forecast + observed archive | **High** | Draft | [`high_prio_gi_draft_pp_decadal_skill_metrics_starved.md`](issues/high_prio_gi_draft_pp_decadal_skill_metrics_starved.md) | Observed in the Tajik (tjhm) backfill 2026-06-17; PENTAD recovered on the same run, so decade-specific. Distinct from ~~PP-036~~ (that fix shipped; starvation persists). Root cause not yet confirmed. |
...
| **PP-042** | Skill recalc: ensemble-exclusion set uses display-form names against DB-form `model_short` (latent) | **Medium** | Draft | [`mid_prio_gi_draft_pp_skill_ensemble_exclusion_form_consistency.md`](issues/mid_prio_gi_draft_pp_skill_ensemble_exclusion_form_consistency.md) | Found 2026-07-07 during LT Skilled-Mean NSE>0 validation (PR #405). Latent — not currently manifesting; defensive fix. Prior aggregates could re-enter the aggregate pool. |
...
| **FD-018** | `month_0` forecasts are annotated with **lead-1** skill (wrong lead attribution) | **Medium** | Draft | [`mid_prio_gi_draft_dashboard_month0_lead_skill_attribution.md`](issues/mid_prio_gi_draft_dashboard_month0_lead_skill_attribution.md) | Found 2026-07-08; pre-existing on `maxat_sapphire_2`, not a regression. `_get_data_monthly` reuses the lead-1-filtered `forecast_stats` for the month_0 block and merges without `horizon_value`. Related: PP-038. |
