...
**Status**: Draft, revision 2 — **not approved, no code written**
**Date**: 2026-08-18 (rev 2 after out-of-loop review returned "not safe to execute as written")
**Scope change in rev 2**: the original Phase 4 (making `validate_pipeline` consume the authority)
is **split out into INFRA-028** — the review showed it was a placeholder, not a phase, and that the
input it needs is a run-scoped manifest rather than a re-derived schedule. **This plan is now only
the behavior-preserving extraction (P0–P3) plus its contract gate.** It ships no user-visible
change: after it lands, the scheduler behaves identically and nothing new consumes the authority.
**Owner decision this implements**: INFRA-022 § "OPEN DECISION — which schedule authority?", option
**(d)** — extract the mode-activity decision into `iEasyHydroForecast` beside the resolver; have both
`lt_schedule_query` and `validate_pipeline` call it.
**Related**: INFRA-021 (must ship atomically with the gating), INFRA-022 (the gating itself),
INFRA-020 (independent; but see § "Synergy with INFRA-020" — one artifact could serve both).
...
> - `model_scheduled_this_month(today, issue_day, forecast_months) -> bool` — **model-level**, called
>   once per model by the caller's existing loop, preserving lazy order
...
- `ISSUE_DAY_TOLERANCE` — moved from `lt_schedule_query.py:52`, currently 10.
  **Rev 2 correction:** this single-sources the value between the *scheduler* and future consumers
  ...
  (`lt_utils.py:196`) and the migration path defines its own `_ALWAYS_SKIP_MODES`
...
- **Work**: two groups.
  ...
  **(a) Outcome matrix** — {inside window, `dist == tolerance`, `dist == tolerance + 1`,
  ...
  clamping in short months, an invalid month list leaving no candidate, several models where only
  one qualifies, **zero models**, and an unknown `horizon_type` (active, but contributes no skill
  type)}. Assert `active_modes`, the **exact** `skipped_modes` reason strings, and
  `skill_metric_types`.
  ...
  **(b) Call-order and exception characterization** — added in rev 2, because this is what a
  ...
  models' getters are **never called**; and each of `get_operational_issue_day`,
  `get_models_to_run`, `get_forecast_months`, `nearest_scheduled_issue_date` and `get_horizon_type`
  retains its current propagate-or-suppress behavior. Also pin **ordering**: `active_modes` follows
...
- **Acceptance**: unit tests cover the P0 matrix at predicate level; the module imports with **no**
  ...
  explicit probe, not an assertion in prose. A built **non-editable wheel** exposes the module in
  both package (`iEasyHydroForecast.long_term_schedule`) and flat-module import styles. Nothing
  else in the repo imports it yet.
...
- **Files**: `apps/long_term_forecasting/lt_schedule_query.py`.
- **Depends on**: P2.
- **Agents**: 1.
...
- **Acceptance**:
  ...
  - A **freshly built** `ltforecast` image — editable installs in an existing image do not prove
    wheel packaging (`iEasyHydroForecast/pyproject.toml:40-48`). Then the Docker smoke at
    `run_docker_tests.sh:661-701`, which invokes the CLI with `--today 2026-03-25` and parses
    `active_modes` at `:683-693`. Extend it with a no-active-modes date so the smoke covers both
    verdicts, and note it currently **skips silently when the image is absent** — a skip must not
    be read as a pass.
...
3. **A fourth definition survives this plan.** `bin/utils/migration_py/long_forecast.py:69-72`
   defines its own `_ALWAYS_SKIP_MODES = {"monthly"}`, applied at `:201`. Decide deliberately
   whether that stays an independent migration rule or is folded in later — otherwise the
   "single-sourced" claim is overstated.
