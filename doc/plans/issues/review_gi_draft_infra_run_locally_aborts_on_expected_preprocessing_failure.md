...
(e.g. `apps/run_locally.sh:1539`, Phase 1; `:1546`, Phase 2). The idiom now
...
**Phase 2** (`run_maintenance_preprocessing_runoff`, `:1546`) — maintenance
preprocessing — not Phase 1 (`run_preprocessing_runoff`, `:1539`, which had
already passed).
...
(`apps/run_locally.sh:1308`) now takes an optional mode suffix, and the
...
call, `run_api_validation "daily"` at `run_locally.sh:1606`, not per-mode.

The per-mode **MODULE** rows were not fixed. `run_machine_learning`
(`apps/run_locally.sh:717`, called from Phase 3) always truncates
`${ERROR_DIR}/machine_learning.log` and always calls
`record_result "machine_learning" ...` with no mode suffix.
`run_maintenance_machine_learning` (`apps/run_locally.sh:998`, called from
...
(`apps/run_locally.sh:1532`) loops `for mode in PENTAD DECAD` in both
...
(`apps/run_locally.sh:496`) skips the mode that doesn't match `ML_MODE`, and
