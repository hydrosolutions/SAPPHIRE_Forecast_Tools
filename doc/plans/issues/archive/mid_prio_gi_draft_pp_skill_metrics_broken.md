...
**Status**: Bug 3 monthly reopened and fixed 2026-03-26. Bug 1 closed (not-a-bug). Bug 2 out of scope. Bug 3 decad closed. Bug 3 monthly fixed (q50 fallback to q).
**Branch**: develop\_long\_term\_fix\_api\_postprocessing\_forecasts
...
| Bug 3 monthly (n\_pairs=0) | **REOPENED 2026-03-26 — code bug, not a data gap.** The 2026-03-24 conclusion was wrong: non-MC\_ALD models DO produce monthly forecasts with real `q` values (verified via DB dump). The actual root cause is `skill_metrics.py:1091`: `merged["forecasted_discharge"] = merged["q50"].astype(float)`. Non-MC\_ALD models store their forecast in `q`, not `q50` (which is NULL). MC\_ALD is the only model that populates `q50`. Fix: `merged["q50"].fillna(merged["q"])`. Implemented 2026-03-26 with 4 new tests in `TestPointForecastFallback`. | **Fixed** |
