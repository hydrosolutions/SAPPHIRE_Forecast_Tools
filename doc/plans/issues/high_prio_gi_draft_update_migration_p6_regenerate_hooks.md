...
- [x] No edits to the four underlying hook scripts (`backfill_snow_stats_history.sh`, `yearly_runoff_hydrograph_aggregation.sh`, `yearly_skill_metrics_recalculation.sh`, `bimonthly_long_term_skill_metrics_recalculation.sh`).
...
| Risk                                                                                   | Mitigation                                                                                                         |
|----------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------|
| SIGKILL / power-loss during a real run leaves cron paused                              | Documented in runbook §7.6 with the manual recovery command (`crontab <backup>`). Backup file is under the log dir. |
