...
If a consumer ever appears, add it then — but the two prospective ones named in earlier drafts have
both since shipped without it: **INFRA-030** shipped its own `SKIP` renderer for skipped modules, and
**LTF-011** (PR #493, `4f171a50`) shipped its split refusal as `FAIL (REFUSED)`. Neither became
`DEGRADED`'s consumer. It is about fifteen lines in one function, and building it now would mean
shipping code that nothing reaches.
